...
def addHydrogens(atomList, *args):
	...
	if atomList:
		if len(atomList[0].molecule.coordSets) > 1:
			replyobj.error("Adding H-bond-preserving hydrogens to "
	...
	typeInfo4Atom, namingSchemas, hydrogenTotals, idatmType, hisNs, \
							coordinations, inIsolation = args
	inversionCache = {}
	addAtoms = {}
	for atom in atomList:
		addAtoms[atom] = 1
	replyobj.status("Categorizing heavy atoms", blankAfter=0)
	problemAtoms = []
	fixed = []
	flat = []
	aroNrings = {}
	...
	for a, crds in coordinations.items():
		...
		for crd in crds:
			hbondInfo.setdefault(crd, []).append((False, a))
	for atom in atomList:
		...
		if numBonds >= substs:
			if atom.element.number == 7 and numBonds == 3 and geom == planar:
				for ring in atom.minimumRings():
					if ring.aromatic():
						if aroNrings.has_key(ring):
							aroNrings[ring].append(atom)
						else:
							aroNrings[ring] = [atom]
			continue
	...
	for needCoplanar in [False, True]:
		...
		for atom in atoms:
			...
			if altlocHandlingNeeded:
				Hpositions = addAltlocHyds(atom, geom, bondLength, needCoplanar, problemAtoms)
			else:
	...
	replyobj.status("Finding hydrogen bonds", blankAfter=0)
	donors = {}
	acceptors = {}
	...
	replyobj.status("Sorting hydrogen bonds by distance", blankAfter=0)
	sortableHbonds = []
	for d, a in hbonds:
	...
	sortableHbonds.sort()
	replyobj.status("Organizing h-bond info", blankAfter=0)
	hbonds = {}
	ambiguous = {}
	relBond = {}
	...
	prunedBy = {}
	reexamine = {}
	replyobj.status("Adding hydrogens by h-bond strength", blankAfter=0)
	while len(processed) < len(hbonds):
		replyobj.status("Adding hydrogens by h-bond strength (%d/%d)" % (len(processed), len(hbonds)), blankAfter=0)
		processedOne = False
		seenAmbiguous = {}
		for dsq, isAcc, hbond in sortableHbonds:
	...
	for a in aroAmines:
		...
		for isAcc, other in hbondInfo.get(a, []):
			...
			badAngle = False
			checkPlanar = typeInfo4Atom[a].geometry == planar
	...
	for a in candidates.keys():
		...
		if len(hbInfo) > 1:
			...
			# the rotamer
			away2, dist, nearA = findNearest(atPos, a, [towardAtom], _nearDist)
			if debug:
		...
		rooms = roomiest(remaining, a, _roomDist, _typeInfo(a))
	...
	for a in candidates.keys():
		...
		if openings > 2:
			...
			if geom == 4 and openings == 3:
				...
				away, dist, awayAtom = findNearest(atPos, a, [], _nearDist)
		...
		if openings > 3 and away is not None:
			...
			away2, dist, nearA = findNearest(atPos, a, [awayAtom], _nearDist)
			if debug:
				print "and away from",
				if nearA:
					print nearA.oslIdent(), "[%.2f]" % dist,
		...
		_attachHydrogens(a, roomiest(positions, a, _roomDist, _typeInfo(a))[
					:hydsToPosition])
	...
	for ns in multiNrings.values():
		...
		if len(positions) == 1:
			...
			if debug:
				print "protonate roomiest ring", roomiest(positions, Npls, _roomDist, _typeInfo)[0][0].oslIdent()
			_attachHydrogens(*roomiest(positions, Npls, _roomDist, _typeInfo)[0])
...
def _attachHydrogens(atom, positions, altLoc=None, occupancy=None):
	...
	for Hpos in positions:
		h = newHydrogen(atom, count, totalHydrogens, namingSchema,
					mtx.apply(Hpos), _typeInfo(atom), altLoc=altLoc)
		if not h:
			# metal clash
			continue
...
def _tryFinish(atom, hbondInfo, finished, aroAmines, prunedBy, processed):
	...
	for isAcc, other in hbondInfo[atom]:
		...
		if len(protons) == hydsToPosition:
			# prevent adding an extra proton sometimes when an H-bond partner
			# becomes fully determined between one call to this routine and the next
			break
		...
		for isAcc, other in conflicting:
			...
			if key in prunedBy:
				for phb in prunedBy[key]:
					...
					hbondInfo.setdefault(phb[0], []).append((False, phb[1]))
					hbondInfo.setdefault(phb[1], []).append((True, phb[0]))
				del prunedBy[key]
...
def addAltlocHyds(atom, geom, bondLength, needCoplanar, problemAtoms):
	altLocs = set()
