...
def model_surfnet(mol, useMesh=True, interval=1.0, cutoff=10,
	...
	surfmol, refMol = _surfnet.model_surfnet(mol, cutoff=cutoff,
							threshold=interval)
	print '%d spheres' % len(surfmol.atoms)
	if refMol is not mol:
		raise RuntimeError("model_surfnet: mismatched molecules")
...
def atoms_surfnet(osl, useMesh=True, interval=1.0, cutoff=10,
	...
	surfmol, mol = _surfnet.atoms_surfnet(atomList, cutoff=cutoff,
						threshold=interval)
	print '%d spheres' % len(surfmol.atoms)
	mList = show_surfnet(surfmol, useMesh, interval,
			     density, color, sameAs=mol)
