...
# --- UCSF Chimera Copyright ---
#
# $Id: acceptorGeom.py 41990 2019-01-15 23:19:50Z pett $
...
import base
from commonGeom import testPhi, getPhiPlaneParams, testTheta, sulphurCompensate, testTau
...
def accThetaTau(donor, donorHyds, acceptor, upsilonPartner, r2,
						upsilonLow, upsilonHigh, theta, tau=None, tauSym=None):
	if base.verbose:
		print "accThetaTau"
	dp = donor.xformCoord()
	...
	if upsilonPartner:
		...
		for bs in bisectors[1:]:
			...
			if testThetaTau(acceptor, dp, donorHyds, ap, bs,
						upsilonLow, upsilonHigh, theta, tau, tauSym):
				return 1
	return testThetaTau(acceptor, dp, donorHyds, ap, upPos,
						upsilonLow, upsilonHigh, theta, tau, tauSym)

def testThetaTau(acc, dp, donorHyds, ap, pp, upsilonLow, upsilonHigh, theta, tau, tauSym):
	...
	if base.verbose:
		print "upsilon okay"
	return testTheta(dp, donorHyds, ap, theta) and testTau(tau, tauSym, acc, ap, dp)
