...
# --- UCSF Chimera Copyright ---
#
# $Id: base.py 41990 2019-01-15 23:19:50Z pett $
...
acceptorParams = [
	...
	[[[('Npl', 'N3'), ['Car', H, H]], [1,1,1,1]],
		(0,), lambda *args: accThetaTau(*args, tau=37.5, tauSym=4), ((1,), 3.42, 90, -137, 140)],
	# waddah
...
def findHBonds(models, intermodel=True, intramodel=True, donors=None,
	...
	randomAccessTypes = (dict, set, WeakKeyDictionary, WeakValueDictionary, WeakSet)
	if donors and not isinstance(donors, randomAccessTypes):
		limitedDonors = set(donors)
	else:
		limitedDonors = donors
	if acceptors and not isinstance(acceptors, randomAccessTypes):
	...
	for model in models:
		...
		metalGroup = model.metalComplexGroup(create=False)
		if metalGroup:
			from chimera.elements import metals
			for pb in metalGroup.pseudoBonds:
				a1, a2 = pb.atoms
				if a1.element in metals:
					metalCoord.setdefault(a2, []).append(a1)
				if a2.element in metals:
					metalCoord.setdefault(a1, []).append(a2)
		...
		for i in range(len(donAtoms)):
			...
			for accModel in models:
				...
				for accAtom, geomFunc, args in accs:
					...
					# ensure not in conflict with a metal-coordination bond
					if accAtom in metalCoord:
						conflict = False
						from chimera import angle
						for metal in metalCoord[accAtom]:
							if angle(metal.xformCoord(), accAtom.xformCoord(),
									donorAtom.xformCoord()) < 90.0:
								conflict = True
								break
						if conflict:
							if verbose:
								replyobj.message("\tH-bond conflicts with metal coordination to %s\n" % metal.oslIdent())
							continue
