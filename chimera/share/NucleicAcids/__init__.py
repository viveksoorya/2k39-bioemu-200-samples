...
def addStyle(name, info, _skipCheck=False):
	...
	for mol in _data:
		if mol in _needRebuild:
			continue
		rds = _data[mol][RESIDUES]
		for rd in rds.itervalues():
			slab_params = rd.get('slab params', None)
			if slab_params and slab_params['style'] == name:
				_needRebuild.add(mol)
				break
...
def drawCylinder(radius, ep0, ep1, **kw):
	...
	# use axis and cos tests to avoid floating point errors
	if axis.sqlength() > 0:
		angle = math.acos(cos)
		t.rotation = (axis[0], axis[1], axis[2], angle)
	elif cos < 0:
		t.rotation = (1, 0, 0, math.pi)
	c = vrml.Cylinder(radius=radius, height=height, **kw)
	t.addChild(c)
	return t

def drawSphere(radius, pt, **kw):
	t = vrml.Transform()
	t.translation = pt.data()
	s = vrml.Sphere(radius=radius, **kw)
	t.addChild(s)
	return t

def skipNonStandardResidue(r):
	if r.type not in nucleic3to1:
		return True
...
def drawSlab(residue, style, thickness, orient, shape, showGly):
	...
	try:
		...
		# Degenerate somehow, maybe linear or NaNs
		return None
	...
	if shape == 'box':
		...
		t.scale = 1, 1, halfThickness * _SQRT2 / radius
		b = vrml.Cylinder(radius=radius, height=(ury - lly),
				**color_kwds)
	elif shape == 'ellipsoid':
		# need to reach anchor atom
		sr = (ury - lly) / 2 * _SQRT3
		t.scale = ((urx - llx) / 2 * _SQRT3 / sr, 1,
				halfThickness * _SQRT3 / sr)
		b = vrml.Sphere(radius=sr, **color_kwds)
	t.addChild(b)
...
def fill5Ring(atoms, c5p=None):
	...
	result = [f]
	for b in fake_bonds:
		node = drawCylinder(radius, pts[b[0]], pts[b[1]], bottom=False, top=False, **color_kwds)
		result.append(node)
	t = vrml.Transform()
...
def sugarTube(residue, anchor=SUGAR, showGly=False):
	...
	nodes = [
		drawCylinder(radius, ep0, ep1, bottom=False, **color_kwds),
		drawSphere(radius, ep0, **color_kwds),
	]
...
def set_slab(side, molecules, residues, style=default.STYLE, **slab_params):
	...
	for r in residues:
		...
		if cur_side == side:
			...
			and tube_params == rd.get('tube params', None)):
				continue
		_needRebuild.add(r.molecule)
...
def ladder(molecules, residues, rungRadius=0, showStubs=True, skipNonBaseHBonds=False, useExisting=False, distSlop=0.0, angleSlop=0.0):
	...
	for a0, a1 in bonds:
		...
		if skipNonBaseHBonds and any(nonBase):
			continue
		r0 = a0.residue
		r1 = a1.residue
		...
		if not c3p1:
			continue
		if rungRadius and not any(nonBase):
			radius = rungRadius
		...
		key = (r0, r1)
		if key in depictBonds:
			prev_radius = depictBonds[key][2]
			if prev_radius >= radius:
				continue
		depictBonds[key] = (c3p0, c3p1, radius, nonBase)

	matchedResidues = set()
	for (r0, r1), (c3p0, c3p1, radius, nonBase) in depictBonds.iteritems():
		...
		try:
			...
			isPurine0 = False
			isPurine1 = False
		if any(nonBase) or isPurine0 == isPurine1:
			mid = 0.5
		elif isPurine0:
		...
		nodes = [
			drawCylinder(radius, c3p0[1], midpt, top=False, **r0color_kwds),
			drawCylinder(radius, c3p1[1], midpt, top=False, **r1color_kwds)
		]
		if r0 in result:
...
def restoreState(mdata, sdata={}):
	for name, info in sdata.items():
		addStyle(name, info, _skipCheck=True)
	for mid in mdata:
...
def _restore_scene(trigger, closure, scene):
	...
	for mid in mdata:
		...
		for rid in srds:
			...
			rd[BASE] = []
		_needRebuild.add(m)
	for m in list(_data.iterkeys()):
		if m in mols or m.__destroyed__:
			continue
		md = _data[m]
		residues = _data[m][RESIDUES].keys()
		# unhide any atoms we would have hidden
		setHideAtoms(False, AlwaysRE, BackboneRE, residues)
		md[RESIDUES].clear()
		_needRebuild.add(m)
...
chimera.triggers.addHandler(chimera.SCENE_TOOL_SAVE, _save_scene, None)
chimera.triggers.addHandler(chimera.SCENE_TOOL_RESTORE, _restore_scene, None)
