...
class Atom:
	...
	def __init__(self, aname, rtype, chainId, rseq, iCode, x, y, z):
		self.anameOrig = aname
		self.aname = aname.strip()
		self.rtype = rtype.strip()
		self.chainId = chainId
		self.rseq = rseq
		self.insertCode = iCode
		self.x = x
		self.y = y
		self.z = z

	def makeChimera(self, mol, atomicNumber):
		e = chimera.Element(atomicNumber)
		...
		try:
			self.chimeraAtom.charge = self.charge
			self.chimeraAtom.autodockType = self.autodockType
...
class Compound:
	...
	def __init__(self, chimeraMol=None, filetype=None, number=0):
		...
		self.atoms = {}
		self.atomList = []
		self.atomNameLeftJustified = True
		self.conect = []
		self.header = ''
		self.compnd = ''
		self.author = ''
		...
		if filetype == "Dock 4, 5 or 6":
			...
			self._dockLine = 0
			self._dockRE = re.compile("""\s*([^=]+)\s*=\s*(\S+)""")
		elif filetype == "AutoDock":
			self._parseDock = self._parseAutoDock
		else:
			self._parseDock = self._parseDock4
		...
		if chimeraMol and chimeraMol.pdbHeaders:
			...
			if chimeraMol.pdbHeaders.has_key('AUTHOR'):
				for line in chimeraMol.pdbHeaders['AUTHOR']:
					...
					self.rawText.append(line)
			self.text = '\n'.join(self.rawText)
		elif chimeraMol and hasattr(chimeraMol, "maeText"):
			self.chimeraModel = chimeraMol
			self.text = chimeraMol.maeText
			self.fields.update(chimeraMol.maeAttr)
			del chimeraMol.maeText
			del chimeraMol.maeAttr
		elif chimeraMol and filetype == "GOLD":
			self.chimeraModel = chimeraMol
			self.data = chimeraMol.mol2data
			self._mineGold(chimeraMol.mol2data, number)
		elif chimeraMol:
			self.chimeraModel = chimeraMol
			try:
	...
	def readPDB(self, f, defaultRseq):
		...
		while 1:
			...
			if not line:
				break
			if line[:3] == 'TER' or line[:6] == 'ENDMDL':
				if not self.data:
					continue
				self.data.append(line)
				break
			elif line[:6] == 'ATOM  ':
				self._pdbParseAtom(line, False)
				self.data.append(line)
			elif line[:6] == 'HETATM':
				self._pdbParseAtom(line, True)
				self.data.append(line)
			elif line[:6] == 'CONECT':
	...
	def _pdbParseAtom(self, line, isHet):
		...
		y = float(line[38:46])
		z = float(line[46:54])
		a = Atom(aname, rtype, chainId, rseq, iCode, x, y, z)
		try:
			...
			a.charge = charge
			a.autodockType = line[77:79]
		self.atoms[serial] = a
		self.atomList.append(a)
		# Check location of single letter atom names.
		# If it is the first letter in the field, assume
		# that all atom names are left justified.
		if aname[0].isspace():
			self.atomNameLeftJustified = False
	...
	def _pdbParseCompnd(self, line):
		...
		self.fields["Compound"] = self.compnd
		self.textArray.append(line)
	...
	def _parseAutoDock(self, line, prefixLen):
		# Ignore prefixLen since AutoDock pdbqt remark records
		# never have serial numbers
		prefixLen = 6
		keepRemark = 1
		field = line[prefixLen:].split(':')
		if len(field) == 2:
			...
			if key == 'VINA RESULT':
				...
				if len(values) == 3:
					...
					self.fields["Score"] = values[0]
					self.fields["RMSD l.b."] = values[1]
					self.fields["RMSD u.b."] = values[2]
		else:
			# So far "VINA RESULT" is the only known
			# record with values
			pass
		if keepRemark:
			self.textArray.append(line)
	...
	def _parseDock35xSearch(self, line, prefixLen):
		...
		if self._dockLine == 0:
			...
			if len(field) == 2:
				...
				replyobj.warning("bad Dock 3.5 field line")
		self._dockLine += 1
		if self._dockLine > 1:
			self._dockLine = 0
		return 1
	...
	def _mineGold(self, data, number):
		for lineno, line in enumerate(data):
			if line.lower().strip() == "@<tripos>comment":
				break
		else:
			...
			return
		import re
		pat = re.compile(r"> <Gold\.(?P<param>[^>]+)>\s*")
		def valueOf(value):
			return _value('\n'.join(value).strip())
		name = None
		value = list()
		text = list()
		for line in data[lineno + 1:]:
			m = pat.match(line)
			if m is not None:
				if name:
					self.fields[name] = valueOf(value)
				name = m.group("param")
				value = list()
			else:
				value.append(line)
	...
	def getAtomType(self, a):
		atomTypes = []
		if self.atomNameLeftJustified:
			if a.aname == a.rtype:
				# eg CA == CA or HG == HG
				atomTypes.append(a.aname)
			atomTypes.append(a.aname[:1])
			atomTypes.append(a.aname[:2])
		else:
			aname = a.anameOrig
			if (len(aname) == 4 and aname[0] == 'H') \
			or aname[0] in "0123456789":
				atype = 'H'
			else:
				atype = aname[:2].strip()
				atype = atype[0].upper() + atype[1:].lower()
			if atype[-1] in "0123456789":
				atype = atype[:-1]
			atomTypes.append(atype)
		for name in atomTypes:
			try:
				return elements.name.index(name)
			except ValueError:
				pass
		# Default to lone pair
		return elements.name.index("LP")
	...
	def buildChimera(self):
		...
		for a in self.atomList:
			a.makeChimera(mol, self.getAtomType(a))
			key = (a.rtype, a.rseq, a.insertCode, a.chainId)
			try:
				res = rMap[key]
			except KeyError:
				res = mol.newResidue(a.rtype, a.chainId,
							a.rseq, a.insertCode)
				rMap[key] = res
		...
		if not self.connected:
			...
			# Delete any cross chain bonds
			interchainBonds = list()
			for b in mol.bonds:
				a0, a1 = b.atoms
				r0 = a0.residue
				r1 = a1.residue
				if r0 != r1 and not r0.id.sameChain(r1.id):
					interchainBonds.append(b)
			while interchainBonds:
				mol.deleteBond(interchainBonds.pop())
...
class Results:
	...
	def __init__(self, filename, filetype, notifySelectionChange,
		...
		if sessionData is not None:
			...
			for line in f:
				...
				index += 1
		elif filetype == "Maestro/Glide":
			replyobj.status("opening %s..." % filename)
			import maestro
			mList = maestro.open_maestro(filename,
							addProperties=True)
			if mList:
				chimera.openModels.add(mList)
			if len(mList) <= 1:
				raise chimera.UserError(
						"no ligands in maestro file %s"
						% filename)
			for i, mol in enumerate(mList[1:]):
				self.compoundList.append(Compound(mol,
							filetype=filetype,
							number=i+1))
		...
		for c in self.compoundList:
			...
			c.buildLabel()
		if len(keys) == 0:
			replyobj.warning("file %s does not appear to be"
						" DOCK output" % filename)
			keys["Number"] = 1
			for c in self.compoundList:
				c.fields["Number"] = c.chimeraModel.subid
		...
		if sessionData is not None:
			...
			elif keys.has_key('Compound'):
				columnKeys.append('Compound')
			if filetype == "AutoDock":
				for k in [ "Score", "RMSD l.b.", "RMSD u.b." ]:
					if keys.has_key(k):
						columnKeys.append(k)
		...
		if needGroup and len(self.compoundList) > 1:
			from ModelPanel import groupCmd
			groupCmd([ c.chimeraModel for c in self.compoundList
							if c.chimeraModel ],
		...
		for row, c in enumerate(self.displayedCompounds):
			try:
				...
				if c.chimeraModel:
					wanted.append(c)
		if wanted:
			self._diagramMake(wanted)

	def _diagramMake(self, wanted):
		molList = [ c.chimeraModel for c in wanted ]
		def complete(zipData, self=self, wanted=wanted):
			self._diagramComplete(zipData, wanted)
		from StructureDiagram.ws import CDKDepictionService
		CDKDepictionService(finishCB=complete,
					width=self.diagramWidth,
					height=self.diagramHeight,
					molecules=molList)

	def _diagramComplete(self, zipData, wanted):
		replyobj.status("Structure diagram generation complete")
		from StringIO import StringIO
		from zipfile import ZipFile
		zf = ZipFile(StringIO(zipData))
		from PIL import Image
		from PIL.ImageTk import PhotoImage
		# First, we save the images in our cache for later use.
		for i, c in enumerate(wanted):
			try:
				imageData = zf.read("depiction-%d.png" % i)
			except KeyError:
				# Image not generated
				replyobj.info("Image %d is missing\n" % i)
				continue
			...
			if w != self.diagramWidth or h != self.diagramHeight:
				...
				# (In fact, we discard all results since
				# they all have the same image size.)
				break
			self.diagramCache[c] = PhotoImage(image)
		if not self.diagramDisplay:
			# User can turn off diagram column as we're generating
			# the images.
			return
		# Now, we display the images in our UI
		for c in wanted:
			try:
				row = self.displayedCompounds.index(c)
				img = self.diagramCache[c]
				self.uiList.item_configure(row, 0, image=img)
	...
	def movieSetup(self):
		...
		if len(self.movieList) < 2:
			replyobj.error("Not enough compounds for movie mode")
			return 0
		if first is None:
