...
class ViewDock(ModelessDialog):
	...
	def _addHbondColumn(self, selectedAtoms):
		...
		from FindHBond import gui as findHBondGui
		findHBondGui.showUI()
		# HACK ALERT: we should probably not reach into
		# a dialog and set values, but...
		fm = findHBondGui.ui.findMode
		self.hbondUIState = fm.getvalue()
		fm.invoke("inter-model")

	def _addHbondColumnCB(self, trigger, ignore, atoms):
		if self.hbondUIState:
			from FindHBond import gui as findHBondGui
			findHBondGui.ui.findMode.invoke(self.hbondUIState)
			self.hbondUIState = None
		if trigger != None:
			mgr = chimera.PseudoBondMgr.mgr()
			pbg = mgr.findPseudoBondGroup("hydrogen bonds")
	...
	def saveSession(self, trigger, data, f):
		...
		if not chimera.nogui and self.app:
