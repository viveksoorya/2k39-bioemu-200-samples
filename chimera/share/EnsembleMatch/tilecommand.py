...
def tile(models, columns = None, spacingFactor = 1.0,
         independentRotation = True, viewAll = True, label = False):
    ...
    import base
    base.tile(models, spacingFactor, columns, viewAll, label)
...
# -----------------------------------------------------------------------------
# Code for cluster command.
#
#   Syntax: cluster <modelSpec>
#               [atoms <atomSpec>]
#
def cluster_command(cmdname, args):

  if len(args) == 0:
    args = '#'
  from Commands import doExtensionFunc
  doExtensionFunc(cluster, args, specInfo = [('modelSpec','models','models'),])

# -----------------------------------------------------------------------------
#
def cluster(models, atoms=""):

    from Commands import CommandError, check_number
    if len(models) == 0:
      raise CommandError('No models specified')
    import base
    base.cluster(models, atoms)
