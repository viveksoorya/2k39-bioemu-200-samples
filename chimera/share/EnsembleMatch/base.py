...
def tile(modelList, scale=1.1, columns = None, viewAll = True, label = False):
	...
	for o in oStates:
		...
		o.globalXform(xf)
		# Add 2dlabels if requested
		if label:
			m = osModels[o][0]
			t = "%s - %s" % (str(m), m.name)
			x = c / float(cols) + xoffset
			y = (rows - r - 1) / float(rows) + yoffset
			n = "tile_%d_%d" % (r, c)
			cmd = ("2dlabels create %s text '%s' "
				"xpos %.3f ypos %.3f" % (n, t, x, y))
			runCommand(cmd)
		# Move to next tile
		c = c + 1
		if c == cols:
