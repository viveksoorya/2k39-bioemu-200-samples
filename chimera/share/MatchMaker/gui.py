...
# --- UCSF Chimera Copyright ---
#
# $Id: gui.py 40858 2015-12-21 23:59:41Z pett $
...
class MatchMaker(ModelessDialog):
	...
	def Apply(self):
		...
		alignKw = { 'ssFraction': ssFraction, 'ssMatrix': ssMatrix,
					'computeSS': computeSS }
		if self.refDomainRestrictionVar.get():
			curResidues = chimera.selection.currentResidues()
			refDomainResidues = curResidues
		else:
			refDomainResidues = None
		if self.matchDomainRestrictionVar.get():
			curResidues = chimera.selection.currentResidues()
			matchDomainResidues = curResidues
		else:
			matchDomainResidues = None
		try:
			matchInfo = match(cp, matchItems, matrix, alg, gapOpen,
				...
				domainResidues=(refDomainResidues, matchDomainResidues),
		...
		if not self.structSeqVar.get():
			return
		chains = set()
		for matchAtoms, refAtoms, rmsd, fullRmsd in matchInfo:
	...
	def fillInUI(self, parent):
		...
		self.refDomainRestrictionVar = Tkinter.IntVar(parent)
		self.refDomainRestrictionVar.set(False)
		Tkinter.Checkbutton(parent,
			text="Further restrict matching\nto current selection",
			variable=self.refDomainRestrictionVar).grid(row=row, column=0)
		self.matchDomainRestrictionVar = Tkinter.IntVar(parent)
		self.matchDomainRestrictionVar.set(False)
		Tkinter.Checkbutton(parent,
			text="Further restrict matching\nto current selection",
			variable=self.matchDomainRestrictionVar).grid(row=row, column=1)
		row += 1

		seqFrame = Tkinter.Frame(parent)
		seqFrame.grid(row=row, column=0, columnspan=2, sticky="nsew")
		...
		matchGroup.grid(row=row, column=0, columnspan=2, sticky="nsew",
								padx=2)
		row += 1
		...
		Tkinter.Checkbutton(parent, text="After superposition, compute"
			...
			variable=self.structSeqVar).grid(row=row, sticky='w',
			columnspan=2)
		row += 1
	...
	def _chainMatchCB(self):
		...
		for slave in self.uiMaster().grid_slaves(row=0):
			slave.grid_forget()
		ref.grid(row=self.chainsRow, column=0, sticky="nsew")
		match.grid(row=self.chainsRow, column=1, sticky="nsew")
...
class ChainMenus(Tkinter.Frame):
	def __init__(self, master, **kw):
		...
		self.labeledFrame.interior().columnconfigure(0, weight=1)
		self.labeledFrame.interior().rowconfigure(0, weight=1)
		self.emptyLabel = WrappingLabel(self.labeledFrame.interior(),
				text="Choose one or more reference chains in the list on the"
				" left.  To specify multiple pairs, choose multiple reference"
				" chains at once; a menu for choosing the corresponding match"
				" chain will be provided for each reference chain.")
		self.emptyLabel.grid(sticky="ew")
