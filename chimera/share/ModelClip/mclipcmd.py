...
def match_settings(match, axis, origin, coord, slab, thickness):
    ...
    if match in ('near', 'nearfar', 'focal'):
        ...
        near,far = c.nearFar
        ex,ey,ez = c.eyePos(0)
        z = near if match.startswith('near') else c.focal
        origin = (ex,ey,z)	# On near plane or focal plane
        if match == 'nearfar':
            slab = True
            thickness = near-far
        ...
        for plane in planeManager.planes:
            ...
            if len(mlist) != 1:
                from Midas import MidasError
                raise MidasError('match value must be a model id, plane name, or "near", "nearfar" or "focal", got "%s"' % str(match))
            m = mlist[0]
            p = m.clipPlane
