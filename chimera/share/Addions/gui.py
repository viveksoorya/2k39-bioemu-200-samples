...
class AddionsDialog(ModelessDialog):
	name = "addions"
	title = "Addions"
	help = "ContributedSoftware/addions/addions.html"
	...
	def fillInUI(self, parent):
		...
		ionTypes = ValidIonTypes.keys()
		ionTypes.sort()
		self.iontype = "Cl-"
		self.iontypeOption = Pmw.OptionMenu(rcf1, label_text=
			"Ion types:", labelpos = 'w', items=ionTypes,
	...
	def Apply(self):
		...
		if not mols:
			self.enter()
			raise UserError("Must select one or model models to add ions to")
		from Addions import ValidIonTypes
		from AddCharge.gui import checkNoCharges
		checkNoCharges(mols, lambda ur, ua, mols=mols: initiateAddions(
			mols, ValidIonTypes[self.iontype], numions, replyobj.status),
			"ion addition", needGaffTypes=True)
