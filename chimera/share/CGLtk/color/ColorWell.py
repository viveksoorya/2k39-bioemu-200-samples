...
# --- UCSF Chimera Copyright ---
#
# $Id: ColorWell.py 41501 2017-04-21 18:32:01Z pett $
...
def colorPanel(*args, **kw):
	...
	if not _colorPanel:
		...
		_colorPanel = _defaultColorPanel(*args, **kw)
	return _colorPanel
...
def _mapFraction(f, maxVal):
	if f < 0: return 0
	elif f > 1: return maxVal
	return int(f * maxVal + 0.5)

def _tkColor(r, g, b, fieldWidth):
	format = '#' + ('%%0%dx' % fieldWidth) * 3
	return format % (r, g, b)

def rgba2tk(rgba, fieldWidth=4):
	maxVal = (16 ** fieldWidth) - 1
	return _tkColor(_mapFraction(rgba[0], maxVal), _mapFraction(rgba[1], maxVal),
							_mapFraction(rgba[2], maxVal), fieldWidth)
...
class _WellInterior(Canvas, ColorSource):
	def __init__(self, colorWell, *args, **kw):
		...
		ColorSource.__init__(self, self)
...
class ColorWell(Frame, ColorTarget):
	...
	def __init__(self, parent,
	  ...
	  wantAlpha = 1
		...
		self.bind("<Button-1>", lambda e=None, s=proxy(self): e and s.edgeClick(e))
		...
		self._well.bind('<Configure>', lambda e=None, s=proxy(self): s._resizeMarkers())
