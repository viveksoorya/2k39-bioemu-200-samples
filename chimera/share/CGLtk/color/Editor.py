...
# --- UCSF Chimera Copyright ---
#
# $Id: Editor.py 39285 2013-11-18 18:48:09Z conrad $
...
class EditorPalette(Palette.Palette, ColorDnd.ColorSource):
	def __init__(self, editor=None, sourceWidget=None, **kw):
		Palette.Palette.__init__(self, **kw)
		if sourceWidget:
...
class Editor(ColorWellInterface):
	...
	def __init__(self, helpCmd=None, **kw):
		ColorWellInterface.__init__(self, **kw)
		self.mainUI(self)
	...
	def mainUI(self, frame):
		...
		self.colorspaceMenu = Tkinter.OptionMenu(*args)
	...
	def setRGBA(self, fromPalette=0, **kw):
		ColorWellInterface.setRGBA(self, **kw)
	...
	def register(self, well, *args, **kw):
		ColorWellInterface.register(self, well, *args, **kw)
		self.checkNoneOkay()
	
	def deregister(self, well, *args, **kw):
		ColorWellInterface.deregister(self, well, *args, **kw)
		self.checkNoneOkay()
	...
	def setChannel(self, event):
		...
		try:
			value = float(s)
			if (value > 1
			and isinstance(self.colorspace, Palette.RGB)):
				value /= 255
