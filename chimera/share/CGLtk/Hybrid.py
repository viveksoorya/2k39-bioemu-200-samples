...
class Scrollable_List:
  ...
  def __init__(self, parent, heading, height, callback = None):
    ...
    if heading != None:
      lbl = Tkinter.Label(f, text = heading, justify = 'left', anchor = 'w')
...
class Checkbutton:
  ...
  def popup_frame_cb(self):
    ...
    if self.variable.get():
      self.popup_frame.grid(**self.grid_args)
    else:
...
class Scale:
  ...
  #
  def enable(self):
    self.label.config(state=Tkinter.NORMAL)
    self.entry.config(state=Tkinter.NORMAL)
    self.scale.config(state=Tkinter.NORMAL)
