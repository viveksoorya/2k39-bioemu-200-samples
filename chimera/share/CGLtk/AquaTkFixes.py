...
def raiseWindow(widget):
  ...
  if aqua:
    # tkraise() focuses window on aqua Tk 8.5.x.  Avoid that.
    t.wm_attributes('-topmost', 1)
    t.after(0, lambda t=t: t.wm_attributes('-topmost', 0))
  else:
    t.tkraise()
  ...
  def update_menus(self):
    ...
    for i in range(1,n+1):      # Skip first menu = application menu
      ...
      if i-1 < len(self.labels):
        ...
        label.bind('<Enter>', lambda e=None, label=label: self.highlight_cb(label))
        label.bind('<Leave>', lambda e=None, label=label: self.unhighlight_cb(label))
        label.bind('<Any-ButtonPress>',
                   lambda e=None, label=label: self.post_menu_cb(label))
