...
class DelPhiExecutor:
	...
	def __init__(self, afterCB=None, options=None,
				outputFileName='delphi.out'):

		# initialize the base class
		#print self, "Initializing DelPhiExecutor..."
		self.afterCB = afterCB
	...
	def WriteInput(self):
		...
		try:
			...
			raise UserError("cannot create temporary file in %s: %s"
	...
	def RunCommand(self):
		...
		from chimera.SubprocessMonitor import Popen, monitor
		from subprocess import STDOUT
		import os.path
		out = file(os.path.join(self.outputDir,
					self.outputFileName), "wb")
		self.process = Popen([ self.executable, self.prmFileName ],
					stdout=out, stderr=STDOUT,
					cwd=self.outputDir)
		monitor("running DelPhi", self.process,
					title="DelPhi progress",
					afterCB=self.Finished)
	...
	def ShowOutput(self):
		...
		if data:
			...
			print "*** NO OUTPUT FROM DELPHI ***"
		print "--- End DelPhi Output ---"
