...
class Scalebar_Dialog(ModelessDialog):
  ...
  def make_model(self, length, thickness, rgb, label, xy_offset, label_rgb,
    ...
    for name, pos in (('1', (-length/2.0, 0, 0)),
      ...
      a.display = 0
      a.surfaceCategory = 'scalebar'
      atoms.append(a)
    m.atom1, m.atom2, m.label_atom = atoms
    la = m.label_atom
    la.color = background_color()
    la.display = 1                      # Needed for label to be visible
    la.drawMode = la.Dot
    ...
    if model_id == None:
      ...
      id, subid = model_id
    chimera.openModels.add([m], baseId = id, subid = subid, noprefs = True)
  ...
  def frozen_models(self):
    ...
    fmlist = [m for m in self.frozen_model_list
              if not m.__destroyed__ and not unopen_model(m)]
