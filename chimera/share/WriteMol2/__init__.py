...
def writeMol2(models, fileName, status=None, anchor=None, relModel=None,
	...
	if isinstance(models, chimera.Molecule):
		...
		class Jumbo:
			def __init__(self, sel):
				...
				self.name = "(selection)"
				class MetalComplexGroup:
					def __init__(self, pbs):
						self.pseudoBonds = pbs
				self._metalComplexGroup = MetalComplexGroup([pb for pb in sel.pseudobonds()
					if pb.category.startwith("metal coordination")])
				self.metalComplexGroup = lambda s=self, **kw: s._metalComplexGroup
		models = [Jumbo(sel)]
		...
		class Jumbo:
			def __init__(self, models):
				...
				class MetalComplexGroup:
					def __init__(self):
						self.pseudoBonds = []
				self._metalComplexGroup = MetalComplexGroup()
				self.metalComplexGroup = lambda s=self, **kw: s._metalComplexGroup
				for m in models:
					self.atoms.extend(m.atoms)
					self.residues.extend(m.residues)
					self.bonds.extend(m.bonds)
					mcg = m.metalComplexGroup(create=False)
					if mcg:
						self._metalComplexGroup.pseudoBonds.extend(mcg.pseudoBonds)
	...
	for mol in models:
		...
		ATOM_LIST = mol.atoms
		BOND_LIST = mol.bonds
		coordinationGroup = mol.metalComplexGroup(create=False)
		if coordinationGroup:
			BOND_LIST.extend(coordinationGroup.pseudoBonds)
		if skip:
			skip = set(skip)
		...
		for i, bond in enumerate(BOND_LIST):
			...
			aromatic = False
			if isinstance(bond, chimera.Bond):
				rings = bond.minimumRings()
			else:
				rings = []
			for ring in rings:
				if ring.aromatic():
