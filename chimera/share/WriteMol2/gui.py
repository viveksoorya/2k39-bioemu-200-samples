...
class WriteMol2Dialog(SaveModeless):
	...
	def fillInUI(self, parent):
		...
		warning = WrappingLabel(self.clientArea, text="Warning: writing"
			" Amber/GAFF types to Mol2 is provided only for compatibility"
			" with the Amber suite of programs. Chimera cannot read the"
			" resulting files successfully.")
		from CGLtk.Font import shrinkFont
		shrinkFont(warning)
		warning.grid(row=row, column=0, sticky="ew")
		row += 1
		...
		Tkinter.Checkbutton(self.clientArea, variable=self.rigidVar,
			text="Write current selection to @SET section of file"
			).grid(row=row, column=0, sticky="w")
		row += 1
	...
	def Apply(self):
		...
		replyobj.status("Writing multiple models to %s" % path)
		writeMol2(models, path, **kw)
		replyobj.status("Wrote multiple models to %s" % path)
