...
def _pruneResidues(rList, prune, minDist):
	pList = [ r for r in prune.residues if r is not None ]
	if callable(pList):
		pList = pList()
...
def GetAtomList(target, prune = None, pruneDist = 30):
	if type(target) is type([]):
		return target
	rList = [ r for r in target.residues if r is not None ]
	if callable(rList):
		rList = rList()
	...
	aList = []
	for r in rList:
		# r might be None if user selected chains and the
		# residue is in SEQRES but does not have coordinates
		if r:
...
def UnpackIntersurfData(atoms_A, atoms_B):
	...
	for m in mSet:
		if refMol is None:
			...
			refMol = m
		elif m.id == refMol.id and m.subid < refMol.subid:
			refMol = m
	xf = refMol.openState.xform
	xf.invert()
	data = []
	for a in atoms_A:
		if a.molecule == refMol:
			p = a.coord()
		else:
			p = xf.apply(a.xformCoord())
...
def ComputeTetrahedralization(data):
	...
	for point in data:
		child_stdin.write(' '.join([str(x) for  x in point[1]]) + '\n')
	child_stdin.close()

	lines = [ line.strip() for line in child_stdout ]
	child_stdout.close()
	tetrahedra = [ [ int(x) for x in values ]
			for values in [ line.split() for line in lines[3:] ] ]
