...
class Interface(ModelessDialog):
	...
	def fillInUI(self, parent):
		...
		self._saveHandlerID = chimera.triggers.addHandler(
						SAVE_SESSION,
						self.saveSession, None)
		self._molChangedHandlerID = chimera.triggers.addHandler(
						"Molecule",
	...
	def _show_geometry(self, vertices, triangles, vertex_colors, solid=0, refMol=None, track=0):
		import numpy
		...
		if track and self._trackedPiece:
			...
			chimera.openModels.add([sm], sameAs=refMol)
			self._trackedPiece = g
			self._trackedModel = sm
		if refMol is None:
			sm.openState.xform = chimera.Xform.identity()
		self.models[sm] = (vertices, triangles, vertex_colors, solid, refMol)
		return sm, g
	...
	def doApply(self, target, solid, track, selAtoms,
		...
		p, refMol = SurfMaker.UnpackIntersurfData(m1_list, m2_list)
		...
		self._show_geometry(surfPoints, surfTriangles, surfColors, solid, refMol, track)
		if selAtoms:
