...
def surface_projection_coordinates(surfaces, projection_axis, volume):
  ...
  if axis_aligned:
    import Matrix
    o = Matrix.apply_matrix(tf, g.origin)
...
def project_and_mask(zsurf, size, mvol, mijk_to_dijk, sandwich, fill_overlap):
  ...
  for zs in zsurfs:
    ...
    for iter in range(max_layers):
      ...
      _mask.fill_slab(depth, depth2, mijk_to_dijk, mvol, dlimit)
      beyond = depth2
      beyond_tnum = tnum2
...
def array_to_model(mvol, volume, ijk_origin, model_id):
  ...
  g = volume.data
  morigin = apply_matrix(g.ijk_to_xyz_transform, ijk_origin)
  ...
  v = volume_from_grid_data(m, show_data = False, model_id = model_id)
...
def slab_surface(va, ta, na, pad, sharp_edges = False):
  ...
  if sharp_edges:
    ...
    narray[2*nv:,:] = _surface.calculate_vertex_normals(varray, tband)[2*nv:,:]
  else:
