...
from v50 import RemapDialog, reportRestoreError, restoreWindowSize, \
	...
	expandSummary, checkVersion, restoreSurfaces, expandSequentialSummary

import globals # so that various version files can easily access same variables
import chimera

def restoreModelAssociations(associations):
	for mid, models in associations.items():
		owner = idLookup(mid)
		for a in models:
			owner.addAssociatedModel(idLookup(a), propagateAttrs=False)
