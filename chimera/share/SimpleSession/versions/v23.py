...
# --- UCSF Chimera Copyright ---
#
# $Id: v23.py 41486 2017-03-22 00:36:25Z goddard $
...
def restoreCamera(detail, fogColor, viewerBG, viewerHL,
	...
	def delay(arg1, arg2, arg3, fogColor=fogColor, viewerBG=viewerBG,
		...
		for attr, val in viewerAttrs.items():
			if attr in ("viewSize", "scaleFactor"):
				continue
			try:
...
def lowestIdXform(models):
	models = [m for m in models if not m.__destroyed__]
	if len(models) == 0:
		return None
...
def restoreFontInfo(fontInfo):
	...
	preferences.getOption(PREF_LABEL, LABEL_FONT).set(fontInfo['face'])
