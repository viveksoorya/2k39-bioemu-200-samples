...
# --- UCSF Chimera Copyright ---
#
# $Id: modbase.py 38342 2013-02-27 00:37:22Z gregc $
...
def parse_file_sax(infile):
    ...
    for data in [handler.getAlignment(), handler.getReference()] + handler.getDerived():
            ## data will be a 2-tuple with containing two strings.  The
            ## first one is the name of a file and the second is the
            ## URL of that file
    
            ## sometimes, there won't be a URL (and data[1].strip() will
	    ## be None) if the file can be fetched from the PDB
        if data[1].strip():
