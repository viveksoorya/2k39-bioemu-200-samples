...
# -----------------------------------------------------------------------------
#
def save_path_tracer_state(file):

  import gui
  path_tracer_dialog = gui.volume_path_dialog()
...
class Path_Tracer_Dialog_State:
  ...
  def state_from_dialog(self, path_tracer_dialog):
    ...
    if sm and not sm.__destroyed__:
  ...
  def restore_state(self, path_tracer_dialog):
    ...
    if self.placement_button in b2b:
      self.placement_button = b2b[self.placement_button]
...
class Link_State:
  ...
  def create_object(self, id_to_marker):
    ...
    if self.extra_attributes:
      l.extra_attributes = self.extra_attributes
