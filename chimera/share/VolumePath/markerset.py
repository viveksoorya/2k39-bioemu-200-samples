...
# -----------------------------------------------------------------------------
#
def load_marker_set_xml(input, model_id = None):
  ...
  return create_marker_sets(h.marker_set_tuples, model_id)

# -----------------------------------------------------------------------------
#
def create_marker_sets(marker_set_tuples, model_id = None):
  ...
  for set_attributes, marker_attributes, link_attributes in marker_set_tuples:
    ...
    if not model_id is None:
      ms.marker_molecule(model_id = model_id)
...
# -----------------------------------------------------------------------------
#
def marker_sets(model_id = None):

    from chimera import openModels as om, Molecule
    id, subid =  (om.Default, om.Default) if model_id is None else model_id
    mols = om.list(id = id, subid = subid, modelTypes = [Molecule])
    msets = [m.marker_set for m in mols if hasattr(m, 'marker_set')]
    return msets
...
def open_marker_set(path):
    ...
    file = open(path, 'r')
    marker_sets = load_marker_set_xml(file)
    file.close()
...
def enable_session_save():
  ...
  def save_session_cb(trigger, x, file):
    import session
    session.save_path_tracer_state(file)

  from SimpleSession import SAVE_SESSION
  from chimera import triggers
  save_session_trigger = triggers.addHandler(SAVE_SESSION, save_session_cb,
                                             None)
