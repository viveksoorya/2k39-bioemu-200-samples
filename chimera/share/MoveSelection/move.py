...
class Selection_Mover:
    ...
    def translate_xy_mouse_drag_cb(self, viewer, event):
        ...
        if self.previous_xy is None:
            return              # Sometimes mouse down is missed on Mac
        ...
        from chimera import Xform, viewer
        pscale = viewer.pixelScale
        xf = Xform.translation(psize*dx*pscale, -psize*dy*pscale, 0)
    ...
    def translate_z_mouse_drag_cb(self, viewer, event):
        ...
        from chimera import Xform, viewer
        pscale = viewer.pixelScale
        xf = Xform.translation(0, 0, psize*dz*pscale)
...
def pixel_size_at_rotation_center(rotation_center):
    ...
    if c.ortho or rotation_center is None:
        return pixel_size
...
def selected_multiscale_chains():
    ...
    if mm:
        chains = mm.selected_chains(selected_surfaces_only = True)
    else:
        chains = []
    return chains
...
def center_of_rotation(object_groups):
    ...
    for g in object_groups:
        c = g.center()
        if c:
            points.append(g.local_to_global_coordinates(c))
...
def extend_to_secondary_structure(atoms, residues):
    ...
    for r in rlist:
        ...
        pos = r.id.position
        cid = r.id.chainId
        sec = (r.isHelix, r.isStrand)
        p = pos + 1
        while (m,cid,p) in rmap:
            r2 = rmap[(m,cid,p)]
            if (not r2 in rt and
                (r2.isHelix, r2.isStrand) == sec):
                p += 1
                rt[r2] = 1
            else:
                break
