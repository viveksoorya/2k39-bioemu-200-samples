...
class Fit_Map_Dialog(ModelessDialog):
  ...
  def simulated_map(self):
    ...
    import fitmap
    v = fitmap.simulated_map(atoms, res, mwm)
  ...
  def fit_map_in_map(self, mmap, fmap, opt_r, opt_t, atoms = []):
    ...
    except (MemoryError, ValueError), e:
      from chimera import replyobj
      replyobj.error('Out of memory fitting %s in %s due to large size of first map.' % (mmap.name, fmap.name))
      return
    ...
    self.report_map_fit(mmap, fmap, stats)
  ...
  def fit_atoms_in_map(self, atoms, fmap, opt_r, opt_t):
    ...
    self.allow_halt(False)
    if stats:
      self.report_atom_fit(atoms, fmap, stats)
  ...
  #
  def report_map_fit(self, moved_map, fixed_map, stats):
    ...
    import fitmap
    message = fitmap.map_fit_message(moved_map, fixed_map, stats)
  ...
  # Report optimization statistics to reply log and status line.
  #
  def report_atom_fit(self, atoms, volume, stats):

    import fitmap
    message = fitmap.atom_fit_message(atoms, volume, stats)
    from chimera.replyobj import info
    info('\n')
    info(message)
  ...
  def report_transformation_matrix(self, model, map):
    ...
    import fitmap
    message = fitmap.transformation_matrix_message(model, map)
    from chimera import replyobj
  ...
  def report_status(self, status):
    ...
    t = time()
    if t - self.last_status_time < self.status_interval:
      return
