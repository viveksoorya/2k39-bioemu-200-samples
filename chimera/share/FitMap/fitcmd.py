...
def fitmap(atomsOrMap, volume,
  ...
  elif search > 0 and flist:
      flist[0].place_models()
...
def fit_atoms_in_map(atoms, volume, shift, rotate, moveWholeMolecules,
    ...
    mols = list(set([a.molecule for a in atoms]))
    if stats:
        from chimera.replyobj import info, status
        info(F.atom_fit_message(atoms, volume, stats))
        if moveWholeMolecules:
            for m in mols:
                info(F.transformation_matrix_message(m, volume))
        ave = stats['average map value']
        status(', average map value %.4g' % ave, append = True, blankAfter = 0)
...
def fit_map_in_map(v, volume, metric, envelope,
    ...
    from chimera.replyobj import info, status
    info(F.map_fit_message(v, volume, stats))
    info(F.transformation_matrix_message(v, volume))
    cort = me if me == 'correlation about mean' else 'correlation'
    status(', %s %.4f' % (cort, stats[cort]), append = True, blankAfter = 0)
...
def map_fitting_points(v, envelope, local_coords = False):
    ...
    point_to_world_xform = None if local_coords else Xform()
    import fitmap as F
    try:
        points, point_weights = F.map_points_and_weights(v, envelope,
                                                         point_to_world_xform)
