...
from Commands import parse_step, parse_subregion, parse_model_id
from Commands import parse_axis, check_in_place, parse_center_axis
...
def vop_command(cmdname, args):
    ...
    operations = {
        ...
        'minimum': (minimum_op, [vspec, gspec]),
...
def add_op(volumes, onGrid = None, boundingGrid = None,
           ...
           gridSubregion = 'all', gridStep = 1, valueType = None,
           inPlace = False, scaleFactors = None, modelId = None):

    combine_op(volumes, 'add', onGrid, boundingGrid, subregion, step,
               gridSubregion, gridStep, valueType, inPlace, scaleFactors, modelId)
...
# -----------------------------------------------------------------------------
#
def minimum_op(volumes, onGrid = None, boundingGrid = None,
               subregion = 'all', step = 1,
               gridSubregion = 'all', gridStep = 1, valueType = None,
               inPlace = False, scaleFactors = None, modelId = None):

    combine_op(volumes, 'minimum', onGrid, boundingGrid, subregion, step,
               gridSubregion, gridStep, valueType, inPlace, scaleFactors, modelId)
...
def combine_op(volumes, operation = 'add', onGrid = None, boundingGrid = None,
    ...
    for gv in onGrid:
        combine_operation(volumes, operation, subregion, step,
                          gv, gridSubregion, gridStep, value_type,
...
# -----------------------------------------------------------------------------
#
def boxes_op(volumes, markers, size = 0, isize = None, useMarkerSize = False,
             subregion = 'all', step = 1, modelId = None):

    volumes = filter_volumes(volumes)
    check_number(size, 'size')
    check_number(isize, 'isize', allow_none = True)
    subregion = parse_subregion(subregion)
    step = parse_step(step)
    ...
    for v in volumes:
        boxes(v, markers, size, isize, useMarkerSize, step, subregion, modelId)
...
# -----------------------------------------------------------------------------
#
def gaussian_op(volumes, sDev = 1.0, subregion = 'all', step = 1,
                valueType = None, invert = False, modelId = None):
    ...
    for v in volumes:
        gaussian_convolve(v, sDev, step, subregion, value_type, invert, modelId)
...
def morph_op(volumes, frames = 25, start = 0, playStep = 0.04,
    ...
    volumes = filter_volumes(volumes)
    if len(volumes) < 2:
        raise CommandError("Morph requires 2 or more volumes, got %d" % len(volumes))
...
# -----------------------------------------------------------------------------
#
def new_op(name, size = (100,100,100), gridSpacing = (1.0,1.0,1.0),
           origin = (0.0,0.0,0.0), cellAngles = (90,90,90),
           valueType = 'float32', modelId = None):

    size = parse_step(size, require_3_tuple = True)
    if isinstance(gridSpacing,str) and ',' in gridSpacing:
        gridSpacing = parse_floats(gridSpacing, 'gridSpacing', 3)
    elif isinstance(gridSpacing, (int, float)):
        g = float(gridSpacing)
        gridSpacing = (g,g,g)
    if isinstance(origin,str):
        origin = parse_floats(origin, 'origin', 3)
    if isinstance(cellAngles,str):
        cellAngles = parse_floats(cellAngles, 'cellAngles', 3)
    valueType = parse_value_type(valueType)
    modelId = parse_model_id(modelId)

    from numpy import zeros
    shape = list(size)
    shape.reverse()
    a = zeros(shape, dtype = valueType)
    from VolumeData import Array_Grid_Data
    grid = Array_Grid_Data(a, origin = origin, step = gridSpacing,
                           cell_angles = cellAngles, name = name)
    import VolumeViewer
    v = VolumeViewer.volume_from_grid_data(grid, model_id = modelId)
    v.show()
    return v
...
def subtract_op(vol1, vol2, onGrid = None, boundingGrid = False,
    ...
    combine_op(vol1+vol2, 'subtract', onGrid, boundingGrid, subregion, step,
               gridSubregion, gridStep, valueType, inPlace, mult, modelId)
