...
class Volume_Filter_Dialog(ModelessDialog):
  ...
  def fillInUI(self, parent):
    ...
    row += 1
    fw.append((self.gaussian, gf))

    ws = Hybrid.Scale(gf, 'Width ', 1.0, 10.0, 0.0, 1.0)
    ws.frame.grid(row = 0, column = 0, sticky = 'ew')
    ...
    vt = Hybrid.Option_Menu(gf, 'Value type', 'same', 'float32', 'float64')
    vt.frame.grid(row = 1, column = 0, sticky = 'w')
    self.gvalue_type = vt.variable
    ...
    fm = Hybrid.Option_Menu(parent, 'Method',
                            'multiply linear', 'divide linear')
    fm.frame.grid(row = row, column = 0, sticky = 'w')
    row += 1
    self.flatten_method = fm.variable
    fw.append((self.flatten, fm.frame))
  ...
  def apply_filter(self, v, ftype, step, subreg):
    ...
    if ftype == self.gaussian:
      ...
      if sd is None:
        return None, None
      import numpy
      vt = getattr(numpy, self.gvalue_type.get(), None)
      params = [sd, vt]
      ...
      try:
        fv = gaussian_convolve(v, sd, step, subreg, vt, task = task)
      ...
      fv = scaled_volume(v, scale = s, shift = o, type = t, step = step, subregion = subreg)
      params = [s,o,t]
    elif ftype == self.flatten:
      method = self.flatten_method.get()
      from flatten import flatten
      fv = flatten(v, method, step, subreg)
      params = [method]
    else:
      return None, None
  ...
  def update_filtered_volume(self, fv, ftype):
    ...
    if ftype == self.gaussian:
      ...
      g = gaussian_grid(v, sd, value_type = vt, region = region)
      params[:] = (sd,vt)
    elif ftype == self.median:
      i = int(self.median_iterations.value(default = -1))
      if i < 0:
      ...
      if i > params[0]:
        g = median_grid(fv, 3, i - params[0], region = region)
      else:
      ...
      method = self.flatten_method.get()
      from flatten import flattened_grid
      g = flattened_grid(v, method, region = region)
      params[0] = method
    else:
      return
