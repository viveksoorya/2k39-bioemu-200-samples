...
class DemoParserNew(DemoParser):
    ...
    def parseDemo(self):
         ...
         if hasattr(demoObj, 'title'):
             self.demo.title = unicode(demoObj.title)
         else:
             self.demo.title = "Chimera Demo"

         if hasattr(demoObj, 'autodelay')  :   self.demo.auto_delay = int(demoObj.autodelay)
         if hasattr(demoObj, 'image')      :   self.demo.img_src   = unicode(demoObj.image)
         if hasattr(demoObj, 'bg_color')   :   self.demo.img_bg_color = unicode(demoObj.bg_color)
         if hasattr(demoObj, 'autorun_on') :   self.demo.autorun_on_start = True
         if hasattr(demoObj, 'description'):   self.demo.description = unicode(demoObj.description)
         if hasattr(demoObj, 'datadir')    :   self.demo.datadir = unicode(demoObj.datadir)
    ...
    def parsePanel(self, xml_panel):
        ...
        if hasattr(xml_panel,'title'):
            p.setTitle( unicode(xml_panel.title) )
        else:
            p.setTitle( self.demo.title )

        ## handle text
        if isinstance(xml_panel.text,list):
            p.setText( unicode(xml_panel.text[0]) )
        elif isinstance(xml_panel.text, basestring):
            p.setText( unicode(xml_panel.text) ) 

        ## handle commands
        if isinstance(xml_panel.command, list):
            cmd_list = []
            for c in xml_panel.command:
                cmd_list.append( (unicode(c.active)=="True", unicode(c.item)) )    
            p.setCmds( cmd_list )
            
        elif isinstance(xml_panel.command, XMLCommand):
            p.setCmds( [ ( unicode(xml_panel.command.active)=="True", unicode(xml_panel.command.item) ) ] )

        ## handle undos
        if isinstance(xml_panel.undo, list):
            undo_list = []
            for u in xml_panel.undo:
                undo_list.append( (unicode(u.active)=="True", unicode(u.item)) )    
            p.setUndos( undo_list )
            
        elif isinstance(xml_panel.undo, XMLUndo):
            p.setUndos( [ (unicode(xml_panel.undo.active)=="True", unicode(xml_panel.undo.item)) ] )
