...
class DemoEditor(ModelessDialog):
        ...
        if okayed:
            ...
            for path in dialog.getPaths():
                try:
                    ...
                    raise UserError("%s is not a demo source file\n"
                        "(Error from parser: %s)" % (path, unicode(v)))
                self.setSavePath(path)
                self.updateWindowTitle()
    ...
    def loadDemo(self, src_path):
        ...
        if hasattr(demoObj, 'title'):
            self.gTitleOption.set(unicode(demoObj.title) )
        if hasattr(demoObj, 'autodelay'):
            self.gAutoDelayOption.set( int(demoObj.autodelay) )
        if hasattr(demoObj, 'image'):
            self.gImageOption.set( unicode(demoObj.image) )
        if hasattr(demoObj, 'datadir'):
            self.gDataDirOption.set( unicode(demoObj.datadir) )
        ...
        for p in demoObj.panels:
            ...
            if hasattr(p, 'autodelay'):
                demo_panel.delay_var.set(unicode(p.autodelay))
            if hasattr(p, 'title'):
                demo_panel.name_var.set(unicode(p.title))
                
            ##re-constitute text state
            panel_text = ''
            if p.text:
                panel_text = unicode(p.text)
            ...
            if p.command:
                if isinstance(p.command, XMLCommand):
                    ...
                    cmds = [ ([False,True][p.command.active=="True"], unicode(p.command.item).strip()) ]
                elif isinstance(p.command, list):
                    for c in p.command:
                        if not hasattr(c,'active'): c.active="True"
                        cmds.append( ([False,True][c.active=="True"],unicode(c.item).strip()) )
            ...
            if p.undo:
                if isinstance(p.undo, XMLUndo):
                    ...
                    undos = [ ([False,True][p.undo.active=="True"], unicode(p.undo.item).strip()) ]
                if isinstance(p.undo, list):
                    for u in p.undo:
                        if not hasattr(u, 'active'): u.active="False"
                        undos.append( ([False,True][u.active=="True"],unicode(u.item).strip()) )
