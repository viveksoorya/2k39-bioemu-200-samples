...
class Model_State:
  ...
  state_attributes = ('name', 'id', 'subid', 'osl_identifier',
		      'display', 'xform', 'active',
                      ...
                      'clip_plane_origin', 'clip_plane_normal', 'silhouette',
                      'version')
  ...
  def state_from_model(self, model):
    ...
    self.silhouette = model.silhouette
  ...
  def restore_state(self, model):
    ...
    if self.version >= 5:
      model.silhouette = self.silhouette
...
class Arrays_State:
  ...
  def id(self, array, atype):
    ...
    if key in a2id:
      ...
      # Hold reference to array so it is not deleted and reused.
      self.keep_refs.append(array)
    return aid
