...
class Matrix_Value_Statistics:
  ...
  def __init__(self, matrix, bins = 10000):
    ...
    import _volume
    mm = [_volume.minimum_and_maximum(m) for m in matrices]
    self.minimum = m0 = min(mn for mn,mx in mm)
    self.maximum = m1 = max(mx for mn,mx in mm)

    # Min / max usually does not pick up NaN values since they compare false to everything.
    # But if the first array value is NaN the following code will catch it.
    from numpy import isnan, logical_not
    self.has_nan = hn = (isnan(m0) or isnan(m1))
    if hn:
      # Recompute min/max ignoring NaN values.
      mm = [_volume.minimum_and_maximum(m[logical_not(isnan(m))]) for m in matrices]
      self.minimum = m0 = min(mn for mn,mx in mm)
      self.maximum = m1 = max(mx for mn,mx in mm)
    ...
    for m in matrices:
      _volume.bin_counts(m, bins_start, bins_end, counts)
    self.counts = counts
    self.bins = bins
...
def surface_level_enclosing_volume(matrix, volume, tolerance = 1e-3,
                                   max_bisections = 30):
  import _volume
  l0, l1 = _volume.minimum_and_maximum(matrix)
  for s in range(max_bisections):
    level = 0.5*(l0 + l1)
    from _contour import surface
    try:
      varray, tarray = surface(matrix, level, cap_faces = True,
