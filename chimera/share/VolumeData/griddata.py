...
class Grid_Data:
  ...
  def __init__(self, size,
               ...
               file_type = '',
               grid_id = '',
               default_color = None):	# R,G,B,A float 0-1.
  ...
  # ---------------------------------------------------------------------------
  #
  def set_ijk_to_xyz_transform(self, ijk_to_xyz):
    t = ijk_to_xyz
    x,y,z = [(t[0][a],t[1][a],t[2][a]) for a in (0,1,2)]
    o = (t[0][3],t[1][3],t[2][3])
    from Matrix import norm, cell_angles_and_rotation
    step = norm(x), norm(y), norm(z)
    ca, r = cell_angles_and_rotation((x,y,z))
    self.set_step(step)
    self.set_cell_angles(ca)
    self.set_rotation(r)
    self.set_origin(o)
    ...
    if m is None and not from_cache_only:
      try:
        ...
        if e.errno == errno.ENOENT:
          msg = ('\n\n%s\n\nYou deleted a file that is still open in Chimera. '
                 'To allow fast initial display of volume data Chimera does not read '
                 'all data from the file when it is first opened, and will later '
                 'read more data when needed. '
                 'Chimera got an error trying to read from a file probably because '
                 'it was deleted after opening it.\n\n') % e.filename
          e.strerror += msg
        raise
