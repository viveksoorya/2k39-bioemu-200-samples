...
file_types = (
  ...
  ('IMAGIC density map', 'imagic', ['imagic'], ['hed', 'img'], False),
  ...
  ('VTK structured points, ascii', 'vtk', ['vtk'], ['vtk'], False),
  )

# -----------------------------------------------------------------------------
#
import mrc, netcdf, cmap, dsn6, imagic
file_writers = (
  ...
  ('Chimera map', 'cmap', '.cmap', cmap.write_grid_as_chimera_map,
   ('chunk_shapes', 'append', 'compress', 'multigrid', 'anisotropic')),
  ('BRIX map', 'dsn6', '.brix', dsn6.write_brix, ()),
  ('IMAGIC density map', 'imagic', '', imagic.write_imagic_grid_data, ()),
  )
...
def absolute_path(path):
  ...
  elif isinstance(path, basestring) and len(path) > 0:
    apath = abspath(path)
  else:
    apath = path
  return apath
