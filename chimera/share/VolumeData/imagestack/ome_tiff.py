...
def parse_ome_tiff_header(path):
    ...
    t270 = i.tag[270]
    if isinstance(t270, (list, tuple)):
        for d in t270:
            if d.startswith('<?xml'):
                desc = d
    elif t270.startswith('<?xml'):
        desc = t270
    else:
        desc = None
    if desc is None:
        raise TypeError('OME TIFF file %s tag 270 does not contain XML block starting with "<?xml".\n' % path +
                        'Tag 270 contains %s' + str(t270))
    ...
    for im in r:
        ...
        for p in im:
            if tag_name(p) != 'Pixels':
                continue
            pa = p.attrib
            for key in ('DimensionOrder', 'SizeC', 'SizeT', 'SizeX', 'SizeY', 'SizeZ'):
                if not key in pa:
                    raise TypeError('OME XML image missing parameter %s, got %s' % (key, str(pa)))
            value_type = pa['Type']
            dorder = pa['DimensionOrder']
            sx, sy, sz = [float(pa.get(s,1)) for s in ('PhysicalSizeX', 'PhysicalSizeY', 'PhysicalSizeZ')]
...
def plane_table(dimension_order, nz, nt, nc, path, tdata):
    ...
    for td in tdata:
        ...
        a = td.attrib
        if [key for key in ('FirstC', 'FirstT', 'FirstZ', 'IFD', 'PlaneCount') if key not in a]:
            continue
