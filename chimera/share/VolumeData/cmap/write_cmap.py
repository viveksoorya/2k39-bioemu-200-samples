...
def write_grid_data(h5file, grid_data, g, settings, progress):
    ...
    arrays = make_arrays(h5file, g, grid_data.size, grid_data.step, atom, settings)
...
# -----------------------------------------------------------------------------
#
def make_arrays(h5file, g, size, spacing, atom, settings):
    ...
    aniso = settings.get('anisotropic')
    istep, jstep, kstep = subsample_step((1,1,1), spacing, aniso)
    ...
    while (isize >= istep and jsize >= jstep and ksize >= kstep and
        ...
        for csname in chunk_shapes:
            cshape = chunk_shape(shape, csname, chunk_elements)
            if not cshape in cshapes:
                a = h5file.createCArray(g, 'data_%s_%d%d%d' % (csname,istep,jstep,kstep), atom,
                                        shape, chunkshape = cshape,
                                        filters = filters)
                a._v_attrs.subsample_spacing = array((istep,jstep,kstep), int32)
                arrays.append(((istep,jstep,kstep), a))
                cshapes[cshape] = True
        istep, jstep, kstep = subsample_step((istep,jstep,kstep), spacing, aniso)

    return arrays

# -----------------------------------------------------------------------------
#
def subsample_step(step, spacing, aniso):
    if aniso:
        s = min(st*sp for st,sp in zip(step,spacing))
        sstep = tuple((2*st if st*sp < 2*s else st) for st,sp in zip(step,spacing))
    else:
        sstep = tuple(2*st for st in step)
    return sstep
