...
class Gaussian_Cube:
  ...
  def __init__(self, path):
    ...
    if have_orbitals:
      ...
      self.orbital_numbers = onums = fields[1:]
      # Handle orbital numbers spilling onto next line in Gaussian09 output.
      norb = int(fields[0])
      while len(onums) < norb:
        onums.extend(f.readline().split())
