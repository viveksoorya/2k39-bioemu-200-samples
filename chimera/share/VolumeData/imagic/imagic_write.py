...
def write_imagic_grid_data(grid_data, path, options = {}, progress = None):
    ...
    #
    # Warning: overwrite files
    #
    if not (split[1] == '.hed' or split[1] == '.img'):
        import Tkinter
        import tkMessageBox
        if (os.path.exists(hed_path) or os.path.exists(img_path)):
            answer = tkMessageBox._show(type='yesno',title='Overwrite existing file',message='File %r exists. Overwrite ?' % path,icon='warning')
            if answer == False:
                return
...
def imagic_header(grid_data, value_type, location, densmin, densmax, avdens, sigma):
    ...
    if value_type == float32:         type = 'REAL'
    elif value_type == int32:         type = 'LONG'
    elif value_type == int16:         type = 'INTG'
    elif value_type == int8:          type = 'PACK'
    ...
    name_string     = '3D VOLUME EXPORTED BY CHIMERA'
    name_array      = [name_string]
    name_array.extend([' '*51])
    name            = ''.join(name_array)

    hist_string     = 'CHIMERA'
    hist_array      = [hist_string]
    hist_array.extend([' '*221])
    history         = ''.join(hist_array)
