...
class Surface_Zone_State:
  ...
  def state_from_zone_table(self, zone_table):
    ...
    zt = {}
    for model, (points, distance, max_components) in zone_table.items():
      zt[(model.id, model.subid)] = (array_to_string(points, floatc),
                                     distance, max_components)
    self.zone_table = zt
  ...
  def restore_state(self):
    ...
    for surface_id_subid, params in self.zone_table.items():
      (points, distance, max_components) = (params if self.version >= 3
                                            else (params[0], params[1], None))
      ...
      if surfs:
        ...
        update = True   # Session only saves auto-update zones.
        surface_zone(surfs[0], npoints, distance, update, max_components)
      else:
