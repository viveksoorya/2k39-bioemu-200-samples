...
# if charge models updated, also update _cmdChargeModel
# also, if new forcefield files (both parm and frcmod),
# check for iron parameters; if there are params then
# modify MMMD.MMTKinter.chargeModelToFiles to only add
# our heme-iron frcmod if needed
...
AMBER14SB = "AMBER ff14SB"
defaultChargeModel = AMBER14SB
currentChargeModels = [AMBER14SB, AMBER03ua, AMBER03_r1, AMBER02pol_r1]
defunctChargeModels = [AMBER99SB, AMBER99bsc0]
knownChargeModels = allChargeModels = currentChargeModels + defunctChargeModels
unitedAtomChargeModels = [AMBER03ua]
oldChargeModels = [AMBER14SB] # leaprc only found in 'oldLeapDir' folder
oldLeapDir = "oldff"
...
def initiateAddCharges(models=None, method=None, gaffType=True, status=None,
	...
	"""add AMBER/GAFF charges to models
	   ...
	   'memorize' and 'memorizeName' are used by other tools (such as
	   Minimize) that call into AddCharge, so that settings chosen by
	   the user can be reused later without asking again.
...
def addStandardCharges(models=None, status=None, phosphorylation=None,
	...
	addAttributes(attrFile, models=models, raiseTool=False)
	...
	for resType, residues in unchargedResTypes.items():
		...
		while True:
			...
			for a in bondResidues[0].atoms:
				if a.bonds or a.name in isolatedNames:
					continue
				isolatedNames.append(a.name)
...
def addNonstandardResCharges(residues, netCharge, method="am1-bcc",
	...
	varieties = {}
	for r in residues:
		primaryAtoms = set(r.molecule.primaryAtoms())
		atomMap = {}
		for i, a in enumerate([a for a in r.oslChildren() if a in primaryAtoms]):
			atomMap[a] = i
		bonds = []
		for a in r.atoms:
			if a not in primaryAtoms:
				continue
			i1 = atomMap[a]
			for n in a.neighbors:
				if n not in primaryAtoms:
					continue
...
def _nonStdCharge(residues, netCharge, method, gaffType, status, showCharges):
	r = residues[0]
	if status:
		status("Copying residue %s" % r.type)
	...
	atomNames = set()
	primaryAtoms = set(r.molecule.primaryAtoms())
	ratoms = [a for a in r.atoms if a in primaryAtoms]
	...
	writeMol2([nm], anteIn, status=status, temporary=True)

	from AmberInfo import amberHome, amberBin
	anteOut = os.path.join(tempDir, "ante.out.mol2")
	command = [os.path.join(amberBin, "antechamber")]
	...
	command.extend([
		...
		"-dr", "n"])
	if status:
		status("Running ANTECHAMBER for residue %s" % r.type)
	...
	if status:
		status("Reading ANTECHAMBER output for residue %s" % r.type)
	...
	if status:
		status("Assigning charges for residue %s" % r.type)
	...
	for ta, ca in zip(templateAtoms, chargedAtoms):
		...
		if ca.charge:
			nonZero = True
	# it is okay for O2 and similar species to be all zero charge...
	if not nonZero and len(set([a.element for a in chargedAtoms])) > 1:
		_clean()
		raise ChargeError("Failure running ANTECHAMBER for residue %s\n"
			"Check reply log for details\n" % r.type)
	...
	for fa, ta in atomMap.items():
		if ta in extras:
			continue
		locs = fa.allLocations()
		for loc in locs:
			loc.charge = ta.charge
			if showCharges:
				loc.label = "%+g" % loc.charge
			if gaffType:
				loc.gaffType = ta.gaffType
		track.addModified(locs, ATTR_SET)
	# map charges onto remaining residues
	rChildren = [a for a in r.oslChildren() if a in primaryAtoms]
	for rr in residues[1:]:
		if r.molecule == rr.molecule:
			rrPrimaryAtoms = primaryAtoms
		else:
			rrPrimaryAtoms = set(rr.molecule.primaryAtoms())
		rrChildren = [a for a in rr.oslChildren() if a in rrPrimaryAtoms]
		for fa, ra in zip(rChildren, rrChildren):
			locs = ra.allLocations()
			for loc in locs:
				loc.charge = fa.charge
				if showCharges:
					if ra.charge is None:
						loc.label = "None"
					else:
						loc.label = "%+g" % ra.charge
				if gaffType:
					loc.gaffType = fa.gaffType
			track.addModified(locs, ATTR_SET)
	_clean()
	if status:
		status("Charges for residue %s determined" % r.type)
...
def _ngCharge(atom):
	...
	if not c2:
		return 1
	Ngpluses = [a for a in c2.neighbors if a.idatmType == "Ng+"]
	if len(Ngpluses) == 2:
		return 1
	heavys = 0
	for nb in atom.neighbors:
...
def _n2Charge(atom):
	# needed in order to get nitite ions correct
	for nb in atom.neighbors:
		if nb.idatmType != "O2-":
			return 0
	return 2
	...
	for r in deletes:
		...
		for p in r.atomsMap['P']:
			...
			if o is None:
				...
				raise UserError("Atom P in residue %s is not connected"
					" to remainder of residue via an oxygen" % str(r))
			v = p.coord() - o.coord()
			sn = getattr(p, "serialNumber", None)
			r.molecule.deleteAtom(p)
			v.length = 0.96
			addAtom("HO5'", chimera.Element('H'), r, o.coord() + v,
	...
	for r in residues[1:]:
		...
		if numAtoms != len(r.atoms):
			raise MidasError("Residues have differing number of"
				" atoms (%d != %d)" % (numAtoms, len(r.atoms)))
	if not chimera.nogui:
		from AddH.gui import checkNoHyds
...
def cmdAddAllCharge(molecules=None, method="am1", chargeModel=None, gaffType=True):
	from Midas.midas_text import MidasError
	if molecules is None:
		molecules = chimera.openModels.list(
						modelTypes=[chimera.Molecule])
...
def _cmdChargeModel(chargeModel):
	...
	if chargeModel:
		...
		elif cm in ["3", "ff03", "03.r1", "ff03.r1"]:
			chargeModel = AMBER03_r1
		elif cm in ["14sb", "ff14sb"]:
			chargeModel = AMBER14SB
		else:
			raise MidasError("Unknown charge model: '%s'; must be"
				" one of 14sb, 03ua, 03.r1, 99sb, or 99bsc0"
				% chargeModel)
	return chargeModel

def unchargedAtomsWarning(ua, warning=""):
	if ua:
		cm = getattr(ua.values()[0][0].molecule, "chargeModel", None)
		if cm in unitedAtomChargeModels:
			atomDeletions = []
			keyDeletions = []
			for key, atoms in ua.items():
				t, n = key
				if n[0] == 'H' and n[1:2].isupper():
					atomDeletions.extend(atoms)
					keyDeletions.append(key)
			if keyDeletions:
				for key in keyDeletions:
					del ua[key]
				for a in atomDeletions:
					r = a.residue
				...
				replyobj.info("Pruned %d hydrogens inappropriate for %s force field\n"
						% (len(atomDeletions), cm))
	if ua:
...
class FakeAtom:
	def __init__(self, atom, res, nameMod=None):
	...
	def __getattr__(self, attrName):
		if attrName == "name" and self.fa_nameMod:
			return "%s-%s" % (getattr(self.fa_atom, attrName),
							self.fa_nameMod)
		if attrName == "residue":
			return self.fa_res
...
class FakeRes:
	def __init__(self, name, atoms=None):
		if atoms == None:
			...
			atoms = [FakeAtom(a, self, a.residue.type)
		...
		self.molecule = atoms[0].molecule

	def oslChildren(self):
		def _cmpAtoms(a1, a2):
			if a1.name == a2.name:
				if a1.altLoc != a2.altLoc:
					return cmp(a1.altLoc, a2.altLoc)
				return cmp(a1.serialNumber, a2.serialNumber)
			return cmp(a1.name, a2.name)
		return sorted(self.atoms, _cmpAtoms)
