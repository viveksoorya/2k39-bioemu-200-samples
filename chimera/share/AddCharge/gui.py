...
from prefs import prefs, CHARGE_METHOD, MEMORIZED_SETTINGS
...
class AddChargesDialog(ModelessDialog):
	...
	def __init__(self, process=None, models=None, chargeModel=None,
		...
		from AddCharge import defaultChargeModel, knownChargeModels
		if not chargeModel or chargeModel not in knownChargeModels:
	...
	def Apply(self):
		...
		settings = {}
		settings['chargeModel'] = self.chargeModel.getvalue()
		settings['method'] = self.chargeMethod.getvalue()
		settings['labelStandard'] = self.labelStandardVar.get()
		settings['labelNonstandard'] = self.labelNonstandardVar.get()
		...
		checkNoHyds(mols, lambda mols=mols, settings=settings:
			initiateAddCharges(cb=self.cb, models=mols,
...
class NonstandardChargeDialog(ModelessDialog):
	...
	def __init__(self, cb, unchargedResidues, unchargedAtoms, method,
			status, gaffType, showCharges=False):
		self.cb = cb
...
def checkNoCharges(items, userCB, process, chargeModel=None, needGaffTypes=False):
	def cb(arg1, arg2, checkItems=items, realCB=userCB):
		for i in items:
			if i.__destroyed__:
				return
		realCB(arg1, arg2)

	# convert to list here in case 'items' is a WeakSet, so that the above code
	# doesn't grab permanent hard references to the contained items
	...
	if type(items[0]) == chimera.Atom:
		...
		for item in items:
			...
			for a in item.atoms:
				...
				or (needGaffTypes and not hasattr(a, 'gaffType')):
					break
			else:
				continue
			noCharges.append(item)
	...
	if chargeModelOkay:
		if needGaffTypes:
			gaffMsg = "and GAFF types "
		else:
			gaffMsg = ""
		msg = "Partial atomic charges %smust be completely assigned for" \
			" %s to work correctly.\n" % (gaffMsg, process)
		if needGaffTypes:
			targets = "charges/types"
		else:
			targets = "charges"
		missingPhrase = "have missing %s" % targets
		assignPhrase = "assign %s" % targets
	else:
...
class ChargeMethodOption(Pmw.RadioSelect):
	def __init__(self, parent, label="Charge method:", method=None, **kw):
		Pmw.RadioSelect.__init__(self, parent, buttontype='radiobutton',
					labelpos='w', label_text=label, **kw)
		self.add("AM1-BCC")
		self.add("Gasteiger")
		if method is None:
