...
def smiles2image_opal(smiles, width, height, scale=1.0, **kw):
	from ws import CDKDepictionService
	s = CDKDepictionService(width=width, height=height,
				smiles=smiles, background=False)
	from StringIO import StringIO
	from PIL import Image
	try:
		return Image.open(StringIO(s.data))
	except:
		s.showErrors()
		from chimera import NonChimeraError
		raise NonChimeraError("cannot generate image from SMILES;\n"
					"see Reply Log for more information")

def molecule2image_opal(m, width, height, scale=1.0, **kw):
	from ws import CDKDepictionService
	s = CDKDepictionService(width=width, height=height,
				molecules=m, background=False)
	from StringIO import StringIO
	from PIL import Image
	try:
		return Image.open(StringIO(s.data))
		...
		s.showErrors()
		from chimera import NonChimeraError
		raise NonChimeraError("cannot generate image from molecule;\n"
					"see Reply Log for more information")
