columns = (
	("Authors", 'citation_authors', {'anchor':"nw"}, 'PDB authors of deposited structure'),
	("Date", 'deposition_date', {'anchor':"n"}, 'PDB structure publication date'),
	("Method", 'expMethod', {'anchor':"n"}, 'PDB experiment type used\nfor structure determination'),
	("PubMed", 'pubmedId', {'anchor':"n"}, 'PDB PubMed literature reference for structure'),
	("Resolution", 'resolution', {'anchor':"n"}, 'PDB resolution in Angstroms'),
	("Title", 'title', {'anchor':"nw"}, 'PDB entry title'),
	("Total atoms", 'nr_atoms', {'anchor':"n"}, 'PDB total number of atoms\nin structure (all chains)'),
	("Total residues", 'nr_residues', {'anchor':"n"}, 'PDB total number of residues\nin structure (all chains)'),
	('Chain names', 'chain_names', {'anchor':"nw"}, 'PDB chain names'),
	('Copies', 'chain_copies', {'anchor':"n"}, 'PDB number of copies\nof matched chain'),
	("Polymers", 'npolymers', {'anchor':"n"}, 'PDB number of different\npolymers in structure'),
	('Residues', 'chain_residues', {'anchor':"n"}, 'PDB number of residues\nin matched chain'),
	('Species', 'chain_species', {'anchor':"n"}, 'PDB chain taxonomy'), 
	('UniProt', 'chain_sequence_id', {'anchor':"n"}, 'Uniprot sequence identifier\nof matched PDB chain'),
	('Weight', 'chain_weight',  {'anchor':"n"}, 'PDB molecular weight (Daltons)\nof matched chain'),
	("Ligand formulas", 'ligandFormulas', {'anchor':"nw"}, 'PDB ligand chemical formulas.'),
	("Ligand names", 'ligandNames', {'anchor':"nw"}, 'PDB ligand chemical names.'),
	("Ligand smiles", 'ligandSmiles', {'anchor':"nw"}, 'PDB ligand smile strings.'),
	("Ligand symbols", 'ligandSymbols', {'anchor':"nw"}, 'PDB ligand symbols.'),
	("Ligand weights", 'ligandWeights', {'anchor':"nw"}, 'PDB ligand weights (Daltons).'),
)
...
def fetch_pdb_info(entry_chain_list):
    ...
    try:
        ...
        replyobj.warning("Fetching BLAST PDB info failed: %s" % str(e))
        return {}
    else:
    ...
    if 'errors' in info:
        from chimera import replyobj
        replyobj.warning("Fetching BLAST PDB info had errors: %s" % info['errors'])
    by_entry = {}
