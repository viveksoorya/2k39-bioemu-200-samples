import threading, os, os.path, string
import chimera
...
class ffmpeg_encoder:
    ...
    def __init__(self, updateFn, **param_dict):
        ...
        if param_dict.get('OUT_FILE'):
            from OpenSave import tildeExpand
            self.out_file = tildeExpand(param_dict['OUT_FILE'])
        else:
            self.out_file = param_dict['OUT_FILE'] = os.path.abspath(DEFAULT_OUTFILE)
    ...
    def _buildArgList(self, param_dict):
        ...
        r = param_dict.get('SIZE_RESTRICTION')
        if not r is None:
            arg_list.append('-vf')
            wd,hd = r
        ...
        for param, option, suffix in (('VIDEO_CODEC', '-vcodec', ''),
                                      ('FORMAT', '-f', ''),
            ...
            if param in param_dict:
                ...
                if p:
                    ...
                    arg_list.append(str(p) + suffix)

        q = param_dict.get('QUALITY')
        if q:
            qopt, qval = q
    ...
    def run(self, message_queue):
        ...
        if exit_val == 0:
            ...
            if len(error_lines) < 10:
                ...
                if not error:
                    # No error message when file is not writable.
                    try:
                        f = open(self.out_file, 'w')
                        f.close()
                    except IOError:
                        error = 'Cannot write file %s' % self.out_file
            else:
