...
# --- UCSF Chimera Copyright ---
#
# $Id: parse.py 34096 2011-07-22 22:33:23Z pett $
...
class NonExistentFileError(UserError, IOError):
	pass

def parseFile(filename, fileType, minSeqs=2, uniformLength=True):
	seqs = None
	if isinstance(filename, basestring) \
	and not filename.startswith("http:") \
	and not os.path.exists(filename):
		raise NonExistentFileError("No such file: %s" % filename)
	for parserType, parse in parseFunc.items():
		if fileType and parserType != fileType:
			continue
		replyobj.status("Trying %s parser" % parserType)
		try:
		...
		if seqs and len(seqs) < minSeqs:
			raise UserError("Only one sequence found in %s file"
				...
				% (parserType, filename))
		replyobj.status("%s parser succeeded for %s" % (parserType,
								filename))
		break
	if seqs is None:
	...
	for s in seqs:
		if uniformLength and len(s) != len(seqs[0]):
