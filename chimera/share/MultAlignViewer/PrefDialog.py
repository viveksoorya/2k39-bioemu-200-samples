...
from prefs import LOAD_PDB_NAME_EXACT, LOAD_PDB_NAME_START, \
	...
	ASSOC_ERROR_RATE, REGION_BALLOON_ON, \
...
class PrefDialog(ModelessDialog):
	...
	def addColorScheme(self, colorFile, makeDefault):
		if colorFile not in self.mav.prefs[RC_CUSTOM_SCHEMES]:
			...
			for grp in (self._multSeqResColorGrp, self._singSeqResColorGrp):
				grp._resColorOpt.values.append(colorFile)
				grp._resColorOpt.labels.append(os.path.basename(colorFile))
				grp._resColorOpt.remakeMenu()
		if makeDefault and len(self.mav.seqs) > 1:
			self.mav.prefs[RESIDUE_COLORING] = colorFile
			self._multSeqResColorGrp._resColorOpt.set(colorFile)
	...
	def _addAppearancePage(self, page):
		...
		self._multSeqResColorGrp = ResColoringGroup(agFrame, self, "",
				self.mav.seqCanvas.setColorFunc)
		self._multSeqResColorGrp.grid(row=3, column=0, sticky='nsew')
		...
		self._singSeqResColorGrp = ResColoringGroup(ssgFrame, self, SINGLE_PREFIX,
			self.mav.seqCanvas.setColorFunc)
		self._singSeqResColorGrp.grid(row=3, column=0, sticky='nsew')
	...
	def _addRegionsPage(self, page):
		...
		def regBalloonChange(opt):
			self.mav.prefs[REGION_BALLOON_ON] = opt.get()
			balloon = self.mav.regionBrowser.regionBalloon
			if opt.get():
				balloon['state'] = 'both'
			else:
				balloon['state'] = 'none'

		tkoptions.BooleanOption(page, 6,
					"Show balloon help for regions",
					self.mav.prefs[REGION_BALLOON_ON], regBalloonChange)

		widgetData = [
			("successful matches", MATCH_REG_ACTIVE, MATCH_REG_EDGE,
				MATCH_REG_FILL, self.mav.MATCH_REG_NAME_START),
			("mismatches", ERROR_REG_ACTIVE, ERROR_REG_EDGE,
				ERROR_REG_FILL, self.mav.ERROR_REG_NAME_START),
			("missing structure", GAP_REG_ACTIVE, GAP_REG_EDGE,
				GAP_REG_FILL, self.mav.GAP_REG_NAME_START)
		]
		assocGroup = Pmw.Group(page, tag_text="Structure association")
		assocGroup.grid(row=7, column=0, columnspan=2)
		...
		for i, regPrefs in enumerate(widgetData):
			descript, activePref, edgePref, fillPref, regNameStart = regPrefs
			...
			Tkinter.Checkbutton(inside, variable=rv,
				...
				regStart=regNameStart: self._changeRegShownPref(
				var.get(), prefName, regStart)
				#command=lambda var=rv, prefName=activePref:
	...
	def _addHeadersPage(self, page):
		...
		from prefs import al2coFrequencies, al2coConservations, \
			...
			AL2CO_GAP, AL2CO_MATRIX, AL2CO_TRANSFORM, defaults
		...
		class Al2coMatrixOption(EnumOption):
			values = matrixNames
		preferredMat = self.mav.prefs[AL2CO_MATRIX]
		if preferredMat not in matrixNames:
			preferredMat = defaults[AL2CO_MATRIX]
	...
	def _changeRegShownPref(self, shown, prefName, regionNameStart):
		self.mav.prefs.update({prefName: shown})
		for regList in self.mav.regionBrowser.associatedRegions.values():
			for reg in regList:
				if reg.name and reg.name.startswith(regionNameStart):
					reg.shown = shown
