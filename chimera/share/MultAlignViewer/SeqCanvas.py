...
#
# $Id: SeqCanvas.py 42328 2021-08-30 18:09:40Z pett $
...
class SeqCanvas:
	...
	def __init__(self, parent, mav, seqs):
		...
		self.mainCanvas.bind('<4>',
			lambda event=None: self._multiScroll('scroll', -1, 'units'))
	...
	def layout(self):
		...
		for seq in self.seqs:
			self.labelBindings[seq] = {
				'<Enter>': lambda e=None, s=seq: self.mav.status(self.seqInfoText(s)),
				'<Double-Button>': lambda e=None, s=seq: self.mav._editSeqName(s)
			}
...
class SeqBlock:
	...
	def _assocResBind(self, item, aseq, index):
		item.tagBind('<Enter>', lambda e=None: self._mouseResidue(1, aseq, index))
		item.tagBind('<Leave>', lambda e=None: self._mouseResidue(0))
