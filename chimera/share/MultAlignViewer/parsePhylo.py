...
# --- UCSF Chimera Copyright ---
#
# $Id: parsePhylo.py 40289 2014-11-05 00:45:57Z pett $
...
class PhyloNode:
	...
	def assignSeqIndices(self, seqs, assignments=None):
		...
		for i, s in enumerate(seqs):
			if self.label.lower().strip() == s.name.replace('_', ' ').lower(
			).strip():
				if self.numbering is None \
				or self.numbering == s.numberingStart:
					thisAssign.append(i)
...
class NewickTree(PhyloNode):
	...
	def _leafNode(self, text):
		...
		if self.numberingsStripped:
			numberingPos = node.label.rindex('/')
			numbering = node.label[numberingPos+1:]
			node.label = node.label[:numberingPos]
			try:
				start, end = numbering.split('-')
			except ValueError:
				start = numbering
			try:
				node.numbering = int(start)
			except ValueError:
				raise SyntaxError("Node %s has unexpected trailing numbering"
					" info" % node.label)
		else:
			node.numbering = None
