...
# --- UCSF Chimera Copyright ---
#
# $Id: HeaderSequence.py 40006 2014-07-31 20:52:36Z pett $
...
class FixedHeaderSequence(HeaderSequence):
	...
	def __init__(self, name, mav, vals):
		self.vals = vals
	...
	def reevaluate(self):
		if len(self.mav.seqs[0]) == len(self.vals):
			self[:] = self.vals
...
def registerHeaderSequence(seq, defaultOn=True):
	# since headers get registered in ChimeraExtension.py files,
	# it is possible to get two different class objects created
	# for the same class, so key on name rather than class...
	registeredHeaders[seq.name] = (seq, defaultOn)
