...
# --- UCSF Chimera Copyright ---
#
# $Id: ReorderDialog.py 39672 2014-04-15 22:10:03Z pett $

import chimera
from chimera.baseDialog import ModelessDialog
from chimera import replyobj
from MAViewer import MOD_ALIGN, SEQ_RENAMED
...
class ReorderDialog(ModelessDialog):
	...
	def __init__(self, mav, *args, **kw):
		...
		self.modHandlerID = self.mav.triggers.addHandler(MOD_ALIGN,
						self._alignMod, None)
		self.renameHandlerID = self.mav.triggers.addHandler(SEQ_RENAMED,
						self._seqRenamed, None)
	...
	def destroy(self):
		self.mav.triggers.deleteHandler(MOD_ALIGN, self.modHandlerID)
		self.mav.triggers.deleteHandler(SEQ_RENAMED, self.renameHandlerID)
		self.mav = None
	...
	def _seqRenamed(self, trigName, myData, trigData):
		sel = self.seqList.curselection()
		self.seqList.setlist([s.name for s in self.seqs])
		if sel:
			self.seqList.selection_set(int(sel[0]))
