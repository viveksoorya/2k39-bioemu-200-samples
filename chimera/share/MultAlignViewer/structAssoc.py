...
# --- UCSF Chimera Copyright ---
#
# $Id: structAssoc.py 42033 2019-05-09 19:27:20Z pett $
...
def tryAssoc(alignSeq, mseq, segments, gaps, estLen, maxErrors=6):
	...
	if not valueError and errors == 0:
		return matchMap, errors
	# prune off 'X/?' residues and see how that works...
	if mseq[0] != 'X' and mseq[-1] != 'X' and mseq[0] != '?' and mseq[-1] != '?':
		if valueError:
			raise ValueError, "bad assoc"
		return matchMap, errors
	from copy import copy
	noX = copy(mseq)
	while noX[:1] == 'X' or noX[:1] == '?':
	...
	Xgaps = copy(gaps)
	while offset and offset >= len(Xsegments[0]):
		offset -= len(Xsegments[0])
	...
	while noX[-1:] == 'X' or noX[-1:] == '?':
		...
		estLen -= 1
		tailLoss += 1
	while tailLoss and tailLoss >= len(Xsegments[-1]):
		tailLoss -= len(Xsegments[-1])
		Xsegments = Xsegments[:-1]
		estLen -= Xgaps[-1]
		Xgaps = Xgaps[:-1]
	if tailLoss:
	...
	try:
		if len(aseq) >= estLen:
			noXmatchMap, noXerrors = constrainedMatch(aseq, noX,
							Xsegments, Xgaps, maxErrors)
		else:
			noXmatchMap, noXerrors = gappedMatch(aseq, noX,
					Xsegments, Xgaps, estLen, maxErrors)
	except ValueError:
		if valueError:
			raise
		return matchMap, errors
...
def constrainedMatch(aseq, mseq, segments, gaps, maxErrors):
	# all the segments should fit in aseq

	offsets, errors = _constrained(aseq, segments, [None] + gaps + [None], maxErrors)
	if errors > maxErrors:
		raise ValueError, "bad assoc"
...
def _constrained(aseq, segments, gaps, maxErrors):
	# find the biggest segment (but non-all-X/? segments win over all-X/? segments)
	longest = None
	for i in range(len(segments)):
		seg = segments[i]
		if seg.count('X') + seg.count('?') == len(seg):
			continue
	...
	minErrs = None
	if bsi == 0:
		targetLeftGap = gaps[0]
	else:
		targetLeftGap = None
	if bsi == len(segments)-1:
		targetRightGap = gaps[bsi+1]
	else:
		targetRightGap = None
	for offset in range(leftSpace, len(aseq) - rightSpace - longest + 1):
		errors = 0
		for i in range(longest):
			if seq[i] == aseq[offset+i] or seq[i] == '?':
				continue
			errors += 1
			if errors > maxErrors:
			...
			gapErrs = 0
			if targetLeftGap is not None:
				gapErrs += abs((offset+1) - targetLeftGap)
			if targetRightGap is not None:
				gapErrs += abs(1 + (len(aseq) - (offset+longest)) - targetRightGap)
			if minErrs is None or errors < minErrs or (
					errors == minErrs and gapErrs < minGapErrs):
				minErrs = errors
				minOffset = offset
				minGapErrs = gapErrs
	...
	if bsi > 0:
		leftOffsets, leftErrors = _constrained(aseq[0:minOffset-1],
					segments[:bsi], gaps[:bsi+1], maxErrors - minErrs)
	else:
	...
	if leftErrors + minErrs > maxErrors or bsi + 1 == len(segments):
		...
		rightOffsets, rightErrors = _constrained(
				aseq[minOffset+longest+1:], segments[bsi+1:], gaps[bsi+1:],
	...
	for i in range(len(errList)):
		...
		if bsi+1 < len(segments):
			rightOffsets, rightErrors = _constrained(
				aseq[offset+longest+1:], segments[bsi+1:], gaps[bsi+1:],
...
def _findGaps(mseq):
	...
	for i in range(len(mseq)):
		...
		if prevRes:
			...
			# narrow gap between residues 34 and 61 of chain A in 1jff
			# requires excluding lengths >= 2.678...
			# For consecutively numbered residues we are a lot more
			# loose in distance requirement, so that 3f5g works,
			# where due to some missing atoms the inter-residue bond
			# between residues 109 and 110 is 4.381...
			decimated = False
			if res.id.position - prevRes.id.position - 1 == 0:
				cutoff = 20.0
				# after a zone deletion, the connecting inter-residue
				# bonds may be missing...
				from chimera.resCode import res3to1
				from chimera.misc import principalAtom
				if not connects \
				and res3to1(prevRes.type) != 'X' and res3to1(res.type) != 'X' \
				and (principalAtom(prevRes) == None or principalAtom(res) == None) \
				and len(res.atoms) < 20 and len(prevRes.atoms) < 20:
					for a1 in res.atoms:
						for a2 in prevRes.atoms:
							if a1.coord().sqdistance(a2.coord()) < 100.0:
								decimated = True
								break
						if decimated:
							break
				...
				cutoff = 6.0
			if not decimated and (not connects or connects[0].sqlength() > cutoff):
				gap = res.id.position - prevRes.id.position - 1
				if gap == -1:
					# 1ton/1bil have gaps in insertion codes...
					prevInsert, insert = \
						prevRes.id.insertionCode, res.id.insertionCode
					if insert != ' ' and prevInsert < insert:
						if prevInsert == ' ':
							gap = ord(insert) - ord('A')
						else:
							gap = ord(insert) - ord(prevInsert)
				# CA-only structures may have longer inter-residue bonds...
				caOnly = len(res.atoms) + len(prevRes.atoms) == 2
				if not (gap == 0 and connects and caOnly \
				and connects[0].sqlength() < 45.0):
					# 3ixy chain B has 6.602 CA-CA length
					# between residues 131 and 132
					if gap < 1:
						# instead of jamming everything together and hoping,
						# use 1 as the gap size, since the gap-placement
						# algorithm doesn't actually care about the size of
						# gap except in tie-breaking cases (placing a segment
						# in two different places has the same score).  Using
						# 1 instead of jamming allows 3oe0 chain A to work,
						# where the gap is between a normal-numbered segment
						# and a long insertion that was numbered much higher
						# [well, between insertion and normal numbering]
						gap = 1
					gaps.append(gap)
					estLen += gap
