...
# --- UCSF Chimera Copyright ---
#
# $Id: output.py 39256 2013-11-06 19:40:29Z pett $
...
def _appendNumberings(inSeqs, doCopy):
	...
	for inSeq in inSeqs:
		if doCopy:
			...
			outSeq = inSeq
		if inSeq.numberingStart is None:
			numberingStart = 1
		else:
			numberingStart = inSeq.numberingStart
		outSeq.name = inSeq.name + "/%d-%d" % (numberingStart,
			numberingStart + len(inSeq.ungapped()) - 1)
...
def _saveCB(okay, dialog, pathInfo=None):
	...
	for path, fileType in pathInfo:
		...
		f.close()
		mav.status("Saved %s" % path)
	mav._edited = False
	
def _verifySaveRegion(mav):
	region = mav.currentRegion()
	if region is None:
		raise UserError("No active region in alignment")
	checkList = {}
	for s1, s2, p1, p2 in region.blocks:
		...
		for si in range(i1, i2+1):
			...
			for p in prange:
				checks[p] = True
	if not checkList:
		raise UserError("No alignment sequences in active region")
	...
	for checks in checkList.values():
		...
		compare.sort()
		if compare != standard:
			raise UserError("All sequences in active region must comprise same columns")
