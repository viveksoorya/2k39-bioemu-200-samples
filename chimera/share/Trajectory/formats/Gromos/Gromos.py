...
# --- UCSF Chimera Copyright ---
#
# $Id: Gromos.py 39434 2014-01-09 00:51:58Z pett $
...
class Gromos:
	...
	def sesSave_gatherData(self):
		return self.coordinates.sesSave_gatherData()

	def __getitem__(self, i):
		return self.coordinates[i-1]

	def __len__(self):
		return self.coordinates.steps
...
class Gromos96Coordinates:
	...
	def __init__(self, fname, scaleFactor, steps, sesInfo=None):
		if sesInfo:
			self.coordFileName = fname = sesInfo['trajFiles'][0]
			self.scaleFactor = sesInfo['scaleFactor']
			self.steps = steps = sesInfo['steps']
		else:
			self.coordFileName = fname
			self.scaleFactor = scaleFactor
			self.steps = steps
...
class SkipCommentsFile:
	...
	def __init__(self, fname):
		...
		if hasattr(self.f, 'fileno'):
			...
			self.f = mmap(self.f.fileno(), 0, access=ACCESS_READ)
		else:
