...
def loadEnsemble(inputs, startFrame, endFrame, callback, relativeTo=None):
	...
	replyobj.status("Processing XYZ files", blankAfter=0)
	...
	for f in range(startFrame+1, endFrame+1):
		...
		if not os.path.exists(fname):
			endAdjust += 1
			continue
		replyobj.status("Processing file %s" % fname,
							blankAfter=0)
		try:
			m = readXYZ(fname)[0]
		except Exception, v:
		...
		setID = crdSet1.id + f - startFrame - endAdjust
	...
	try:
		...
		if endAdjust:
			replyobj.status("There were gaps in the numbering of the XYZ files.", color="blue")
		else:
