...
def escape(s):
	return ''.join([ _SpecialCharacters.get(c, c) for c in s ])
...
class Environment:
	...
	def handleLine(self, line):
		...
		if line[0] == "arrow":
			...
			self.handleLine(".draw " + ' '.join([ str(x+dx),
								str(y+dy),
								str(z+dz) ]))
		elif line[0] == "font":
			family, size = line[1].lower(), int(line[2])
			if len(line) > 3: style = line[3].lower()
			else: style = "plain"
			...
			elif style == "bolditalic": self.fontStyle = "BOLDITALIC"
			...
			dx, dy, dz = (float(line[1]), float(line[2]), float(line[3]))
			x, y, z = self.curPosition
			self.handleLine(".move " + ' '.join([ str(x+dx),
								str(y+dy),
								str(z+dz) ]))
		elif line[0] == "polygon":
			...
			if line[2][0] in 'xyzXYZ':
				obj = Object(form='rotate', axis=line[2][0].lower(), angle=radians(float(line[1])))
			else:
	...
	def buildVRML(self):
		...
		for obj in self.lineObjs:
			...
			o <     "}"
			o <     "coordIndex ["
			o <     ' '.join([ str(i) for i in  range(len(obj.vertices)) ])
			o <     "]"
			if obj.vertex_colors[0] is not None:
				...
				o <     "}"
				o <     "colorIndex ["
				o <     ' '.join([ str(i) for i in range(len(obj.vertex_colors)) ])
				o <     "]"
