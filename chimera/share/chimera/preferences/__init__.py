	...
	if files:
		for filename in files:
			try:
				...
				import shutil
				try:
					shutil.copyfile(filename, '%s.save' % filename)
				except IOError:
					pass
				break
			else:
...
if sys.platform != "win32":
	...
	# "access" is not very accurate on Windows, so
	# we use mkdir/rmdir for testing permissions

	def _canWrite(path):
		# If path exists and is writable, we're done
		if os.path.exists(path) and os.access(path, os.W_OK):
			return True
		# Find the longest path that exists and see
		# if we can create a directory beneath it
		head = path
		root = None
		missing = None
		while True:
			...
			if newHead == head:
				...
				break
			missing = newTail
			if os.path.exists(newHead):
				root = newHead
				break
			else:
				head = newHead
		newDir = os.path.join(root, missing)
		try:
			os.mkdir(newDir)
		except OSError:
			return False
...
def setupMatplotlib(reset=False):
	prefFile = preferences.filename()
	if not prefFile:
		return
	mplDir = os.path.join(os.path.dirname(prefFile), ".matplotlib")
	...
	if mplDir:
		import chimeraInit
		chimeraInit.putenv("MPLCONFIGDIR", mplDir)
...
class _PrefCategory(Category):
	def __init__(self, master=None, *args, **kw):
		from chimera.tkoptions import BooleanOption, InfoOption
