...
class SaveUI:
	...
	def updateComboList(self, selectCurrent=True):
		self.presetItems = _mkcasemap(self.client.saveui_presetItems())
		self.userItems = _mkcasemap(self.client.saveui_userItems())
		l = self.presetItems.values() + self.userItems.values()
		l.sort()
	...
	def updateState(self):
		...
		if name.lower() in self.userItems:
	...
	def saveAs(self, name, confirm=True):
		if not name:
			...
			return
		lower = name.lower()
		if lower in self.presetItems:
			name = self.presetItems[lower]
			from chimera import replyobj
			replyobj.error("\"%s\" is a preset name" % name)
			return
		elif lower in self.userItems:
			name = self.userItems[lower]
			if confirm and not self._confirmReplace(name):
				return
		selectCurrent = True
	...
	def _selectCB(self, item):
		if item is None:
			return
		lower = item.lower()
		if lower in self.userItems:
			item = self.userItems[lower]
		elif lower in self.presetItems:
			item = self.presetItems[lower]
		else:
			return
	...
	def _saveAsCB(self):
		...
		items = self.presetItems.values() + self.userItems.values()
		items.sort()
	...
	def _canSave(self, name):
		if name and name.lower() not in self.presetItems:
			return True
		else:
			return False
