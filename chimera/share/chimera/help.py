...
# license details.
#
# $Id: help.py 42229 2020-11-24 17:03:04Z pett $
...
def display(widgetOrURL, package=chimera, newWindow=False):
	...
	if type(path) is unicode:
		path = path.encode('utf-8')  # fix for #17884
	path = urllib.quote(path)
	parameters = urllib.quote(parameters)
	if type(query) is unicode:
		query = query.encode('utf-8')  # fix for #16912
	...
	replyobj.status("See web browser for %s" % url, blankAfter=10)
	if chimera.nogui:
		# Only show help in nogui mode if there is access to a GUI.
		okay = False
		if 'DISPLAY' in os.environ:
			okay = True	# check X11
		if 'SECURITYSESSIONID' in os.environ:
			okay = True	# check Mac OS X
		# check Windows?
		if not okay:
			return
	import webbrowser
...
def makeBalloonWidget(master):
	...
	if master.tk.call('tk', 'windowingsystem') == 'aqua':
		import CGLtk
		btop = balloon.component('hull')
		CGLtk.balloonDontTakeFocus(btop)
	global _balloonWidget
	_balloonWidget = balloon
	return balloon
