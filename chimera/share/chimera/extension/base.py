...
# license details.
#
# $Id: base.py 42088 2019-10-22 19:40:18Z pett $
...
class Manager:
	...
	def __init__(self, *args, **kw):
		...
		self.instMenu = {}
		self.catMenu = {}
		self.additionalToolsMenu = None
		self.tangramDialog = None
		self.settingsGrid = None
		self.settingsButtons = {}
		self.dirList = None
		self.toolbarPlacement = self.defaultToolbarPlacement = "left"
	...
	def loadDirectory(self, path, core=False):
		"Search a path for package Python tools"

		self.processingCoreExtensions = core
		from chimera.misc import stringToAttr
		if path not in sys.path:
			sys.path.append(path)
		self.categoryChanged = {}
		self.newCategoryAdded = 0
		module = "ChimeraExtension"
		try:
		...
		try:
			try:
				...
				if module in sys.modules:
					# Freeing module sets globals to None.
					# Prevent that since code may have
					# registered callbacks that use globals.
			...
			from chimera import replyobj
			import traceback
			message = ("Error while importing %s from %s:\n"
							% (pkgName, path))
			s = traceback.format_exception(*sys.exc_info())
			message += ''.join(s)
			replyobj.error(message)
	...
	def registerExtension(self, emo):
		...
		# EXT REP IMP
		# from repository import manager as repMgr
		# repMgr.addEmo(emo)
	...
	def remakeToolsMenu(self):
		...
		for menu in self.catMenu.itervalues():
			menu.destroy()
		self.catMenu = {}
		if self.additionalToolsMenu:
			self.additionalToolsMenu.destroy()
		...
		for cat in self.sortedCategories():
			...
			self.remakeCategoryMenu(cat)
		self.remakeAdditionalToolsMenu(self.menu)
	...
	def _showTangramDialog(self):
		if not self.tangramDialog:
			from chimera.baseDialog import ModelessDialog
			class TangramDialog(ModelessDialog):
				title = "Insilichem"
				...
				def fillInUI(self, parent):
					...
					iconImage = ImageTk.PhotoImage(
						Image.open(os.path.join(os.path.dirname(__file__), "tangram.jpg")))
					tangramIcon = Tkinter.Label(parent, image=iconImage)
					tangramIcon.photo = iconImage # keep ref
					tangramIcon.grid(row=0, column=0)
					tangramLabel = Tkinter.Label(parent, text= "The Tangram suite of"
						" tools add a wide range of new capabilities to UCSF Chimera.")
					from CGLtk.Font import shrinkFont
					shrinkFont(tangramLabel, fraction=1.5)
					tangramLabel.grid(row=1, column=0)

				def LearnMore(self):
					import webbrowser
					webbrowser.open("https://github.com/insilichem/tangram")
			self.tangramDialog = TangramDialog()
		self.tangramDialog.enter()

	def remakeAdditionalToolsMenu(self, toolsMenu):
		toolsMenu.add_separator()
		atMenu = Tkinter.Menu(toolsMenu, title="Additional Tools")
		insiliMenu = Tkinter.Menu(atMenu, title="InsiliChem/Tangram...")
		atMenu.add_command(label="Insilichem/Tangram", command=self._showTangramDialog)
		atMenu.add_separator()
		import webbrowser
		atMenu.add_command(label="More Chimera plugins...", command=lambda wb=webbrowser:
			wb.open("https://www.cgl.ucsf.edu/chimera/plugins/plugins.html"))
		from chimera import dialogs
		atMenu.add_command(label="Add third-party plugin location...",
			command=lambda dlg=dialogs: dlg.display("preferences").setCategoryMenu("Tools"))
		from chimera import help
		atMenu.add_command(label="Create your own plugin/extension...", command=lambda
			f=help.display: f("ProgrammersGuide/Examples/index.html"))
		toolsMenu.add_cascade(label="Additional Tools", menu=atMenu)
	...
	def createUI(self, master):
		from chimera import tkoptions, toolbar
		class ToolbarPlacementOption(tkoptions.EnumOption):
			"""Specialization of EnumOption Class for side"""
			values = (Tkinter.LEFT, Tkinter.RIGHT, Tkinter.TOP,
					Tkinter.BOTTOM, toolbar.TOOLBAR_RA)
		...
		tbplace = self.toolbarPlacement
		self.toolbarPlacementOption = ToolbarPlacementOption(f, 0,
			"Toolbar placement", tbplace,
			lambda opt: self._updateToolbarPlacement(opt.get()))
		self._updateToolbarPlacement(tbplace)

		w = Pmw.Group(master, tag_text="Third-party plugin locations")
	...
	def _updateToolbarPlacement(self, val):
		self.toolbarPlacement = val
		from chimera.tkgui import app
		app.toolbar.side = val
	...
	def preferencesLoad(self, optDict, loadDir=True):
		...
		if optDict.has_key(TOOLBAR_PLACEMENT):
			self.toolbarPlacement = optDict[TOOLBAR_PLACEMENT]
		else:
			self.toolbarPlacement = self.defaultToolbarPlacement
		...
		if self.dirList:
			self.remakeLocations()
		try:
			self.toolbarPlacementOption.set(self.toolbarPlacement)
		except AttributeError:
			pass
		self._updateToolbarPlacement(self.toolbarPlacement)
	...
	def preferencesSave(self):
		...
		if self.toolbarPlacement != self.defaultToolbarPlacement:
			optDict[TOOLBAR_PLACEMENT] = self.toolbarPlacement
		return optDict
...
class OrderedDirList(OrderedListOption):
	...
	def __init__(self, manager, master=None, *args, **kw):
		from chimera.tkgui import app
		OrderedListOption.__init__(self, master, *args, **kw)
		self._openButton = self.buttonBox.add('Open',
					state=Tkinter.DISABLED,
	...
	def showDir(self):
		path = self.outline.selected.obj
		import os.path
		if not os.path.exists(path):
			from chimera import UserError
			raise UserError("%s no longer exists" % path)
		import chimera
		chimera.help.display("file://%s" % path)

	def deleteAll(self):
		OrderedListOption.deleteAll(self)
		self.manager.updateDirectories(
			[ obj for desc, obj in self.items() ])
	...
	def outlineSelect(self, oEntry):
		if oEntry:
			...
			self._openButton.config(state='normal')
		else:
			self._deleteButton.config(state='disabled')
			self._openButton.config(state='disabled')
...
TOOLBAR_PLACEMENT = "toolbar placement"
...
def setup():
	...
	preferences.addCategory("Tools", ToolsCategory, aliases=["Extensions"],
				manager=manager)
	manager.loadDirectory(chimera.pathFinder().dataRoot, core=True)
