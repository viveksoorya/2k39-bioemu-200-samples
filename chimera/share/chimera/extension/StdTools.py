...
def registerStandardTools():
	...
	for name in StructMeasure.gui.pageNames:
		if name == StructMeasure.gui.BONDROTS:
			continue
	...
	m.registerExtension(ColorActionsEMO(__file__))
	m.registerExtension(EffectsEMO(__file__))
	m.registerExtension(PaletteEMO(__file__))
...
class ColorActionsEMO(base.EMO):
	def name(self):
		return "Color Actions"
	def description(self):
		return "Display color actions dialog"
	def categories(self):
		return [ "Depiction" ]
	def icon(self):
		import chimera
		import os.path
		file = chimera.pathFinder().firstExistingFile("chimera",
					os.path.join("images", "color-actions.png"),
					False, False)
		if not file:
			return None
		return file
	def activate(self):
		from chimera import dialogs
		from chimera.tkgui import ColorActionDialog
		dialogs.display(ColorActionDialog.name)
...
class PaletteEMO(base.EMO):
	def name(self):
		return "Palette Editor"
	def description(self):
		return "Display palette editor"
	def categories(self):
		return [ "Utilities" ]
	def activate(self):
		from chimera import dialogs
		from chimera import paletteoptions
		dialogs.display(paletteoptions.PaletteEditor.name)
