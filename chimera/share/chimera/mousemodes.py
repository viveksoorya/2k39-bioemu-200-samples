...
# license details.
#
# $Id: mousemodes.py 39151 2013-10-11 23:01:06Z pett $
...
MULTITOUCH_GESTURES = 'Use multitouch gestures on Mac'
...
class MouseCategory(preferences.Category):
	...
	def __init__(self, *args, **kw):
		...
		import tkgui
		self.onAqua = tkgui.windowSystem == 'aqua'
		self.__regularOptions = {AUTOSPIN: False, MOUSEWHEEL: True}
		if self.onAqua:
			self.__regularOptions[MULTITOUCH_GESTURES] = True

		global _mouseCategory
		_mouseCategory = self

		self.__autospin = None
		self.__mousewheel = None
		if self.onAqua:
			self.__multitouch = None
	...
	def ui(self, master, rebuild=False):
		...
		val = ro[MOUSEWHEEL]
		self.__mousewheel = tkoptions.BooleanOption(self._ui, 3,
				"Use scrolling", val,
				lambda o, s=self: s._mousewheelCB(o.get()))
		if self.onAqua:
			self.__multitouch = tkoptions.BooleanOption(self._ui, 4,
				MULTITOUCH_GESTURES, ro[MULTITOUCH_GESTURES],
				lambda o, s=self: s._multitouchCB(o.get()))
		return self._ui
	...
	def load(self, optDict, notifyUI=True, updateSaved=1):
		...
		if not self.__mousewheel is None:
			self.__mousewheel.set(ro[MOUSEWHEEL])
		self._mousewheelCB(ro[MOUSEWHEEL])
		if self.onAqua:
			if not self.__multitouch is None:
				self.__multitouch.set(ro[MULTITOUCH_GESTURES])
			self._multitouchCB(ro[MULTITOUCH_GESTURES])
		if notifyUI:
			self._pref.notifyUI()
		if updateSaved:
	...
	def _save(self, saveAll=0):
		...
		if autospin is not None:
			optDict[AUTOSPIN] = autospin
		if self.__mousewheel is None:
			mousewheel = ro.get(MOUSEWHEEL, True)
		else:
			mousewheel = self.__mousewheel.get()
		if mousewheel is not None:
			optDict[MOUSEWHEEL] = mousewheel
		if self.onAqua:
			if self.__multitouch is None:
				multitouch = ro.get(MULTITOUCH_GESTURES, True)
			else:
				multitouch = self.__multitouch.get()
			if multitouch is not None:
				optDict[MULTITOUCH_GESTURES] = multitouch
		return optDict
	...
	def _mousewheelCB(self, value):
		global mousewheel
		mousewheel = value

	def _multitouchCB(self, enable):
		global multitouch
		multitouch = enable
		from tkgui import app
		if app is None or app.graphics is None:
			return
		g = app.graphics
		tpcb = app.trackPadTouchesCB if enable else ''
		g.configure(touchescommand = tpcb)
