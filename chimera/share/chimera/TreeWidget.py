# From intermediate IDLE release (0.4?).  Same copyright as Python.
#
# $Id: TreeWidget.py 35739 2012-03-02 23:33:28Z conrad $
...
#            self.draw(7, 2)
            if self.item._IsExpandable():
                ...
                id = self.canvas.create_image(7, 9, image=image)
    ...
    def selected_items(self):
        if self.selected:
            yield self.item
        subnodes = self.children[:]
        while subnodes:
            node = subnodes.pop(0)
            if node.selected:
                yield node.item
            if node.children:
                subnodes[:0] = node.children
...
class TreeItem:
    ...
    def OnDoubleClick(self):
        ...
        Return false if normal processing should proceed.
        Return true if normal processing should be skipped."""
        return False
