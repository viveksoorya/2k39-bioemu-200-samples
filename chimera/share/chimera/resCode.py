...
class StdResDict(dict):
	def update(self, *args, **kw):
		standard = kw.pop('standard', True)
		if standard:
			oldKeys = set(self.keys())
		dict.update(self, *args, **kw)
		if standard:
			for key in self.keys():
				if key not in oldKeys:
					from chimera import PDBio
					PDBio.addStandardResidue(key)
standard3to1 = StdResDict()
standard3to1.update(nucleic3to1)
standard3to1.update(protein3to1)
origStandard3to1 = standard3to1.copy() # before any MODRES hanky panky

def res3to1(code3):
	try:
		return(standard3to1[code3])
	except KeyError:
		return 'X'
