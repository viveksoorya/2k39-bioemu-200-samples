...
# license details.
#
# $Id: pbgPanel.py 38218 2013-02-05 01:38:37Z pett $
...
from chimera.tkoptions import PseudoBondGroupColorOption, \
	PseudoBondDrawModeOption, BondDisplayModeOption, HalfbondOption, \
	...
	if isinstance(pbg, chimera.ChainTrace):
		if len(pbg.pseudoBonds) > 0:
			return "model %s: chain" % (
						...
						.molecule.oslIdent()[1:],)
	return pbg.name or pbg.category or "unknown"
...
class _GroupInspector(ModelessDialog):
	...
	def _fillInContents(self):
		...
		row += 1
		opts = getOptionsForClass(chimera.PseudoBond)
