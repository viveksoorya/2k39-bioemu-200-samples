...
# license details.
#
# $Id: triggerSet.py 39674 2014-04-16 00:13:06Z pett $
...
class _TriggerHandler:
	...
	def invoke(self, triggerName, triggerData, raiseError):
		import chimera
		try:
...
class _Trigger:
	...
	def activate(self, triggerData, raiseError):
		...
		for handler in self._handlerList:
			...
			if prederegister:
				self._delList.append(handler)
			try:
				ret = handler.invoke(self._name, triggerData, raiseError)
			except:
				self._locked = locked
				raise
		...
		if not self._locked:
			for handler in self._delList:
				self._handlerList.remove(handler)
			self._delList = []
			self._handlerList += self._addList
			self._addList = []
	...
	def release(self):
		...
		for activateData in self._needActivateData:
			self.activate(activateData, self._needActivateMap[id(activateData)])
		self._needActivateData = []
	...
	def listHandlers(self):
		...
		- each list element is a list of [<func>, <funcData>]
		'''
		handlers = []
		for handler in self._handlerList:
			if handler in self._delList:
				continue
			handlers.append( [handler._func, handler._funcData] )
		return handlers
...
class TriggerSet:
	...
	def activateTrigger(self, name, data, absentOkay=False,
		...
		Normally, errors that occur in trigger handlers are caught
		and reported (without actually re-raising the error) so that the
		remaining handlers will execute.  If 'raiseErrors' is set to
		True, then such errors will be reported but then re-raised
		-- so further handlers will not execute.
		"""
		if name not in self._triggerDict:
			if absentOkay:
				return
			raise KeyError, name
		self._triggerDict[name].activate(data, raiseError)
	...
	def isTriggerBlocked(self, name):
		"""Is named trigger blocked?"""
		return self._triggerDict[name].isBlocked()
	...
	def triggerHandlers(self, name):
		"""Return all the handlers for trigger with the given name.

		triggerset.triggerHandlers(name) => list

		If no trigger corresponds to name, an exception is raised.
		"""
		return self._triggerDict[name].listHandlers()

	def triggerNames(self):
		"""Return an unordered list of the trigger names (dict keys).

		triggerset.triggerNames() => list
		"""
		return self._triggerDict.keys()
