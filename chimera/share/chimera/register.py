...
class RegDialog(ModelessDialog):
	...
	def addInfo(self, parent):
		...
		self.info = HtmlText(self.regGroup.interior(), height=height,
			relief=Tk.FLAT, wrap=Tk.WORD, highlightthickness=0)
		if registered:
		...
		self.info.insert(0.0,
			...
			"software is having in the scientific community.  "
			"The information you supply will only be used "
			"for reporting summary usage statistics to NIH "
			"(<i>i.e.,</i> no individual data will be reported "
			"to NIH).")
		self.info.configure(padx=5, pady=5, state=Tk.DISABLED)
	...
	def addFields(self, parent):
		...
		self.entryLabel = Tk.Label(self.entryFrame,
			text="Please provide the following information,"
			    " and click 'Register' ('*' denotes a required"
			    " field).  Chimera will install the registration"
		...
		self.nihChk = Tk.Checkbutton(self.NIHFrame,
			text='This software will be used in support of'
			' NIH-funded research',
		...
		self.usersChk = Tk.Checkbutton(usersFrame,
			  text="Add me to the Chimera users mailing list"
			  " (chimera-users@cgl.ucsf.edu),\n"
			  "which is used for discussion of Chimera usage"
			  " and features",
			  justify=Tk.LEFT, variable=self.usersVar)
		...
		self.announceChk = Tk.Checkbutton(announceFrame,
			  text="Add me to the Chimera announcements mailing"
			  " list (chimera-announce@cgl.ucsf.edu),\n"
			  "which is used for announcing new releases and"
			  " workshops (~2-3 messages per year)",
			  justify=Tk.LEFT, variable=self.announceVar)
	...
	def processResponse(self, res, text):
		if res == 'Invalid':
			...
			replyobj.error("The %s you have entered is not valid."
					"  Please try again.\n" % error)
			return 0
		elif res == 'Error':
			from BugReport import bugNotify
			bugNotify("An error has occurred on the Chimera"
				" registration server.",
			...
			if data[0] == 0:
				...
				replyobj.info(res + '\n')
				self.status(
					"Thank you for registering chimera.",
					echoToMain=True)
				return 1
	...
	def Register(self):
		...
		if doSubscribeUsers:
			try:
				self.subscribe(usersURL, info['user'],
								info['email'])
			except:
		...
		if doSubscribeAnnounce:
			try:
				self.subscribe(announceURL, info['user'],
								info['email'])
			except:
		...
		## pop up the dialog that says Chimera has been registered
	...
	def subscribe(self, url, username, email):
		...
		text = file.read()
		file.close()
		# Return text is HTML and not that useful
		#print text
...
class AlreadyRegisteredDialog(ModelessDialog):
	...
	def fillInUI(self, parent):
		...
		t = HtmlText(parent, width=60, height=7, relief=Tk.FLAT,
								wrap=Tk.WORD)
