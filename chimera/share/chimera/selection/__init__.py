...
# license details.
#
# $Id: __init__.py 41276 2016-09-09 22:17:57Z pett $
...
import chimera
from chimera import SelEdge, SelVertex, SelSubgraph, SelGraph, triggers
import copy
...
SEL_NAMED = "selection named"
DEL_NAMED_SELS = "delete named selections"
triggers.addTrigger(SEL_NAMED)
triggers.addTrigger(DEL_NAMED_SELS)
savedSels = {}
def saveSel(saveName, sel=None, updateGui=True):
	if sel is None:
		sel = copyCurrent()
	update = saveName in savedSels
	savedSels[saveName] = sel
	triggers.activateTrigger(SEL_NAMED, (saveName, update))

def delNamedSel(selName):
	del savedSels[selName]
	triggers.activateTrigger(DEL_NAMED_SELS, [selName])

def delAllNamedSels():
	names = savedSels.keys()
	savedSels.clear()
	triggers.activateTrigger(DEL_NAMED_SELS, names)
...
class Selection:
	...
	def pseudobonds(self, **kw):
		return self._filtered(self.edges(**kw), chimera.PseudoBond)

	def residues(self, **kw):
		if 'ordered' in kw:
			return self._filtered(self.subgraphs(), chimera.Residue)
		# atomsBonds2Residues doesn't preserve order...
	...
	def chains(self, **kw):
		if 'ordered' in kw:
			...
			ordered = False
		if ordered:
			residues = self.residues(ordered=True)
			seqDict = {}
			resDict = {}
		...
		if asDict:
			...
			return chainDict
		if ordered:
			chains.sort(lambda c1, c2: cmp(seqDict[c1], seqDict[c2]))
		return chains
...
class ItemizedSelection(Selection):
	...
	def pseudobonds(self, **kw):
		cacheInfo = kw.items()
		cacheInfo.sort()
		cacheKey = ("pseudobonds", tuple(cacheInfo))
		if not self._cache.has_key(cacheKey):
			self._cache[cacheKey] = Selection.pseudobonds(self, **kw)
		return copy.copy(self._cache[cacheKey])
	...
	def implied(self, vertices=True, edges=True, asDict=False, ordered=False):
		...
		values: arbitrary). If 'ordered' is True, source edges/vertices
		will be handled in sorted order rather than dictionary order.
		"""
		...
		if vertices:
			...
			es = eDict.keys()
			if ordered:
				es.sort(lambda e1, e2: cmp(eDict[e1], eDict[e2]))
			for e in es:
		...
		if edges:
			...
			vs = vDict.keys()
			if ordered:
				vs.sort(lambda v1, v2: cmp(vDict[v1], vDict[v2]))
			for v in vs:
	...
	def addImplied(self, vertices=True, edges=True, ordered=False):
			...
			if level == SelGraph:
				if isinstance(item, chimera.Molecule):
					...
					if item.oslChildren():
						...
						self._barrenGraphs[item] = self._index
						self._index += 1
					continue

			elif level == SelSubgraph:
				vertices = item.oslChildren()
				# add inter-subgraph edges if both vertices selected
				edges = set()
				if subgraphs is None:
					subgraphs = set([sb for sb in it
							 if sb.oslLevel() == SelSubgraph])
				for v in vertices:
					for e in v.oslChildren():
						if e in edges:
							continue
						other = [ev for ev in e.oslParents() if ev != v][0]
						if other in self.items[SelVertex] \
						or other.oslParents()[0] in subgraphs:
							edges.add(e)
				subitems.extend(vertices)
				subitems.extend(list(edges))
				continue

			from _surface import SurfacePiece
			if (isinstance(item, SurfacePiece) and
			    isinstance(item.model, chimera.MSMSModel) and
			    item == item.model.surface_piece):
				surf = item.model
		...
		if method == INTERSECT:
			...
			if method == REPLACE:
				...
				if isinstance(selection, ItemizedSelection):
					...
					values = vertices.values()+edges.values()+bgraphs.values()
					if values:
						self._index = max(values) + 1
					else:
						self._index = 0
			else:
				# maintain ordering
...
class _CurrentSelection(ItemizedSelection):
	...
	def _monitorChangesCB(self, trigName, myData, trigData):
		chimera.triggers.deleteHandler('monitor changes',
							self.monitorHandler)
		self.monitorHandler = None
		# clear out dead items that may be in the next
		# set of triggers...
		for itemDict in self.items.values():
			removeables = [i for i in itemDict.keys() if i.__destroyed__]
			if removeables:
				for rm in removeables:
					del itemDict[rm]
	...
	def add(self, it):
		...
		self.updateGraphics = True
		if not self.monitorHandler:
			# don't fire trigger directly in case there are other pending changes...
			self.monitorHandler = chimera.triggers.addHandler(
				'monitor changes', self._monitorChangesCB, None)
	...
	def addImplied(self, vertices=True, edges=True):
		...
		self.updateGraphics = impV or impE or self.updateGraphics
		if not self.monitorHandler:
			# don't fire trigger directly in case there are other pending changes...
			self.monitorHandler = chimera.triggers.addHandler(
				'monitor changes', self._monitorChangesCB, None)
...
class OSLSelection(Selection):
	...
	def apply(self, vFunc = None, eFunc = None):
		...
		if (vFunc or mySGFunc) and len(vList) > 0:
			vList.sort()
	...
	def barrenGraphs(self, asDict=False):
		...
		oslParser.applyFunctions(self.oslStr, chimera.openModels.list(),
						lambda g, si, ci, mi, l=bgList: l.append((si, ci, mi, g)), None, None)
		bgList.sort()
		if asDict:
			return dict([(x[-1], i) for i, x in enumerate(bgList)])
		return [x[-1] for x in bgList]
...
def currentPseudobonds(**kw):
	"""Return pseudobonds in current selection.

	Takes the same arguments and returns the same values as
	ItemizedSelection.pseudobonds()
	"""
	return _currentSelection.pseudobonds(**kw)
