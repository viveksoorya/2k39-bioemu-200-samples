...
# --- UCSF Chimera Copyright ---
#
# $Id: chain.py 31452 2010-09-20 23:25:39Z pett $
...
for mol in molecules:
		...
		if backbone:
			...
			for name, atomList in atoms.items():
				if not bbAtoms.has_key(name) and atomList[0].element.number > 1:
					selAdd.extend(atomList)
addSet = set(selAdd)
for mol in molecules:
	for a in mol.atoms:
		if a.element.number == 1 and a.neighbors and a.neighbors[0] in addSet:
			selAdd.append(a)
sel.add(selAdd)
