...
# license details.
#
# $Id: misc.py 41166 2016-07-09 01:00:38Z pett $
...
def getPseudoBondGroup(category, modelID=-1, associateWith=[], hidden=True,
							addModel=True, issueHint=False, create=True):
	"""get a pseudobond group regardless of whether it already exists
	   ...
	   'hidden' indicates that the group shouldn't normally appear in
	      ...
	      in Pseudobond Panel)
	   'addModel' controls whether the group will be added to the open
	      model list if it is created.  Typically set to False if
		  the pseudobonds are between models that aren't yet open.
	   ...
	   'create' controls whether the group will be created if it doesn't
	      exist.  None will be returned for a missing group.
	"""

	mgr = chimera.PseudoBondMgr.mgr()
	group = mgr.findPseudoBondGroup(category)
	if not group and create:
...
def chimeraLabel(item, showModel=None, modelName=False, style=None,
	...
	if not style:
		if chimera.nogui:
			...
			import preferences
			from initprefs import PREF_LABEL, ATOMSPEC_CONTENTS, \
					ATOMSPEC_SERIAL, ATOMSPEC_SIMPLE
			preferred = preferences.get(PREF_LABEL, ATOMSPEC_CONTENTS)
...
def isProtein(model):
	...
	import _molecule
	return _molecule.isProtein(model)
...
def processPdbChemName(name, useGreek=True, probableAbbrs=False):
	text = ""
	word = ""
	for c in name.strip().lower():
		...
		if word:
			if c.isdigit() or (text and text[-1].isdigit()):
				text += _processChemWord(word, useGreek, probableAbbrs).upper()
			else:
				text += _processChemWord(word, useGreek, probableAbbrs)
			word = ""
		text += c
	if word:
		if c.isdigit() or (text and text[-1].isdigit()):
...
def _processChemWord(word, useGreek, probableAbbrs):
	if len(word) == 1:
		return word.upper()
	import elements
	if len(word) == 2 and word.capitalize() in elements.name:
		return word.capitalize()
	if set(list(word)) <= set(["i", "v", "x"]):
		# Roman numeral
		return word.upper()
	if useGreek and word in greekLetters:
		return greekLetters[word]
	if probableAbbrs and len(word) < 5 and word != "cell":
		return word.upper()
	segs = []
...
def atomSearchTree(atomContainer, sepVal=5.0, xformed=True, crdSet=None):
	...
	if xformed:
		if crdSet:
			return AdaptiveTree([a.xformCoord(crdSet).data()
				for a in atoms], atoms, sepVal)
		else:
			return AdaptiveTree([a.xformCoord().data()
				for a in atoms], atoms, sepVal)
	else:
		if crdSet:
			return AdaptiveTree([a.coord(crdSet).data()
				for a in atoms], atoms, sepVal)
		else:
			return AdaptiveTree([a.coord().data()
				for a in atoms], atoms, sepVal)
...
def stringToAttr(string, prefix="", style="underscore", collapse=True):
	...
	for c in string:
		...
		if illegal:
			if style == "underscore":
				attrName += "_" + c
			elif attrName and not attrName[-1].isupper():
				attrName += c.upper()
			else:
				attrName += c
		else:
