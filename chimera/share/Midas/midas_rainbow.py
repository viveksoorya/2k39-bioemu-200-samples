...
def rainbowModels(models, changeAt='residues', colors=defaultColors):
	...
	if changeAt == 'helix':
		ranges = []
		for m in models:
			ranges.extend(_getSecondaryStructureRanges(m, ['helix']))
	elif changeAt == 'strand':
		ranges = []
		for m in models:
			ranges.extend(_getSecondaryStructureRanges(m, ['strand']))
	elif changeAt == 'sse':
		ranges = []
		for m in models:
			ranges.extend(_getSecondaryStructureRanges(m, ['helix','strand']))
	elif changeAt == 'models':
...
def _getSecondaryStructureRanges(m, types = ('helix','strand')):
	ranges = []
	cranges = _getChainRanges(m)[0]
	for cres in cranges:
		cr = []
		t = None
		for r in cres:
			if r is None:
				continue
			rt = ('helix', r.ssId) if r.isHelix else (('strand', r.ssId) if r.isStrand else None)
			if rt and rt == t:
				cr[-1].append(r)
			elif rt and rt[0] in types:
				cr.append([r])
				t = rt
			else:
				t = None
