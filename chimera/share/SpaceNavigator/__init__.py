...
class Space_Navigator:
    ...
    def __init__(self):
        ...
        if self.device:
            ...
            self.handler = t.addHandler('new frame', self.check_space_navigator, None)
    ...
    def check_space_navigator(self, trigger_name, d1, d2):
        ...
        if tmag > 0:
            ...
            shift = axis * 0.15 * self.speed * viewer.camera.extent * tmag/512
            xf = Xform.translation(shift)
            self.apply_transform(xf)

        if zmag != 0:
            f = pow(1.1, self.speed*float(zm)/512)
# TODO: Updating zoom depth causes slow frame rate zooming on large surfaces
#  (e.g. 10 million triangles, 2 frames / second).
#            import Midas
