...
# -----------------------------------------------------------------------------
#
def show_only_largest_blobs(p, visible_only = False, blob_count = 1,
                            rank_metric = 'size rank'):

    b = Blob_Masker(p, visible_only)
    m = b.triangle_mask(metric = rank_metric, limit = blob_count)
    import Surface
    Surface.set_triangle_mask(p, m)
    import Surface
    Surface.set_visibility_method('hide dust', p.model , None)
...
class Blob_Masker:
    ...
    def __init__(self, p, visible_only = False):
        ...
        vt = p.geometry if self.tsubset is None else p.maskedGeometry(p.Solid)
        self.varray, self.tarray = vt
        self.vertex_count = len(self.varray)
    ...
    def blob_area(self, vi, ti):
        from MeasureVolume import surface_area
        t = self.tarray[ti,:]
