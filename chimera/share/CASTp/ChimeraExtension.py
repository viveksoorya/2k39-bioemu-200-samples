...
class CastpEMO(chimera.extension.EMO):
	...
	def openID(self, id, ignore_cache=False):
		return self.module("gui").processCastpID(id, ignore_cache=ignore_cache)
...
fetch.registerIdType("CASTp",				# name of database
			...
			"1www", 			# example
			"CASTp ID",			# fetch type
			"sts.bioe.uic.edu/castp/",	# homepage
			"http://sts.bioe.uic.edu/castp/query.php?pdbid=%s&submitid=Search")				# info url
