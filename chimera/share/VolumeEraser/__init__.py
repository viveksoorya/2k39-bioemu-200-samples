...
def writable_volume():
    ...
    from VolumeViewer import active_volume, volume
    v = active_volume()
    if v is None:
        return None         # No volume chosen in volume viewer
    vc = v.writable_copy()
    # Make sure the full region is shown.
    ijk_min, ijk_max, ijk_step = volume.full_region(vc.data.size, ijk_step = v.region[2])
    vc.new_region(ijk_min, ijk_max, ijk_step)
    return vc
