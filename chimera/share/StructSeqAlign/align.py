...
# --- UCSF Chimera Copyright ---
#
# $Id: align.py 42523 2024-06-04 23:40:19Z pett $
...
def multiAlign(chains, cutoff, matchType, gapChar, circular, statusPrefix=""):
	...
	pas = {}
	pairings = {}
	replyobj.status("%sFinding residue principal atoms" % statusPrefix,
							blankAfter=0)
	for seq in chains:
		seqpas = []
		pairing = []
		...
		for seq2 in chains[i+1:]:
			...
			try:
				...
				for i, pa in enumerate(pas[seq2]):
					...
					xyzs.append(pa.xformCoord().data())
					data.append((i, pa))
				trees[seq2] = tree = AdaptiveTree(xyzs, data, cutoff)
	...
	if circular:
		replyobj.status("%sMinimizing circularities" % statusPrefix,
							blankAfter=0)
		circulars = {}
		while 1:
			circularVotes = {}
	...
	replyobj.status("%s Collating columns" % statusPrefix, blankAfter=0)
	...
	for col in orderedColumns[1:]:
		...
		if gap is not None:
			for pos in range(newOrdered[-1].positions[gap]+1,
							col.positions[gap]):
				if not gap.residues[pos % len(gap.residues)]:
					continue
	...
	while colIndex < len(orderedColumns) - 1:
		replyobj.status("%sMerging columns (%d/%d)" % (statusPrefix,
	...
	replyobj.status("%sComposing alignment" % statusPrefix, blankAfter=0)
	...
	clones.sort(lambda a, b: oslModelCmp(a.molecule.oslIdent(),
						b.molecule.oslIdent()))
	replyobj.status("%sDone" % statusPrefix)
	return clones
...
def findPruneCrosslinks(allLinks, pairings, seq1, seq2,
	...
	replyobj.status("%sFinding crosslinks %s" % (statusPrefix, tag),
							blankAfter=0)
	...
	replyobj.status("%sPruning crosslinks %s" % (statusPrefix, tag),
							blankAfter=0)
	while linkList:
		pen = pos = None
