...
#           p = placement, a = all
#
def molecule_copy(from_atoms, to_atoms, settings = 'csv', matchResidues = True):
    ...
    for c, cf in ct:
        if c in settings or ('a' in settings and c != 'y'):
            cf(ma, mb, mr, mm)
...
def match_objects(from_atoms, to_atoms, match_residues):
    ...
    if match_residues:
        def rid(r):
            return (r.id.chainId, r.id.position, r.type)
    else:
        # TODO: Handle overlapping from and to residues.
        rorder = {}
        for alist in (from_atoms, to_atoms):
            rn = 0
            for a in alist:
                r = a.residue
                if r not in rorder:
    ...
    for a in from_atoms:
        r = a.residue
        fat[(rid(r), a.name, a.altLoc)] = a
        ma[a] = []
    for a in to_atoms:
        r = a.residue
        k = (rid(r), a.name, a.altLoc)
        if k in fat:
            ma[fat[k]].append(a)
        else:
            upa += 1
...
def copy_aligned_coordinates(ma, mb, mr, mm):
    ...
    for a, tal in ma.items():
        for ta in tal:
            txfinv = ta.molecule.openState.xform.inverse()
            x = txfinv.apply(a.xformCoord())
            ta.setCoord(x)
