...
def run_mscalc(xyzr, probe_radius = 1.4, vertex_density = 2.0,
    ...
    if mserr and report_stderr:
        info(mserr)

    explain = ('Surface calculation frequently fails for large, multi-chain '
               'structures. The calculation may be successful if the chains '
               'are treated individually, by using the "split" command '
               'before generating a surface.  If splitting is not desired '
               'or the structure is already a single chain, changing '
               'molecular surface parameters in the Selection Inspector '
               'or (before surface creation) the New Surfaces category '
               'of Preferences may allow the calculation to succeed. '
               'More details are given at'
               '\n\n'
               'http://www.cgl.ucsf.edu/chimera/docs/UsersGuide/surfprobs.html')
    try:
        if status != 0:
            m = ('Surface calculation failed, mscalc returned code %d.\n\n%s'
                 % (status, explain))
            raise Surface_Calculation_Error(m)
        ...
        if fallback_to_single_component and all_components:
            results = run_mscalc(xyzr, probe_radius, vertex_density,
                                 all_components = False)
            from chimera.replyobj import warning
            warning('Calculation of some surface components failed.\n\n'
                    'A single component of the surface was calculated '
                    'correctly - the failure occurred for disconnected '
                    ' additional components such as inside bubbles.  If the '
                    'desired surface is already shown, there is no need for '
                    'concern.  However, if it is not shown, see below for '
                    'possible fixes.\n\n' + explain)
            return results
        raise
...
def run_shell_command(args, input):
    ...
    if sys.platform[:3] == 'win':
        ...
        if hasattr(subprocess, 'STARTF_USESHOWWINDOW'):
            flags = subprocess.STARTF_USESHOWWINDOW
        else:
            # Python 2.7 privatized STARTF_USESHOWWINDOW.
            # Didn't seem to offer any other way to hide the console window
            # for a subprocess.
            import _subprocess
            flags = _subprocess.STARTF_USESHOWWINDOW
        spinfo.dwFlags |= flags
    else:
        spinfo = None
