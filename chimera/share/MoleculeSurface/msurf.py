...
class MSMSModel(SurfaceModel):
    ...
    # -------------------------------------------------------------------------
    # This routine allows changing all surface parameters and doing just one
    # surface recalculation.  Setting the parameters individually on the surface
    # would cause a recalculation for each parameter changed.
    #
    def update_parameters(self, probeRadius = None, vertexDensity = None,
                          allComponents = None):

        changed = False
        for name, p in (('probeRadius', probeRadius),
                        ('density', vertexDensity),
                        ('allComponents', allComponents)):
            if not p is None and p != getattr(self, name):
    ...
    def update_coloring(self):
        ...
        if cm == self.ByMolecule:
            m = self.molecule
            if m:
                p.color = molecule_surface_rgba(m)
                p.vertexColors = None
        elif cm == self.ByAtom:
            m = self.molecule
            if m is None or self.atomMap is None:
                return
            rgba = [atom_surface_rgba(va) for va in self.atomMap]
            if rgba:
                p.vertexColors = rgba
    ...
    def set_custom_colors(self, vcolors):
        ...
        vrgba = [c.rgba() for c in vcolors] if not vcolors is None else None
    ...
    def surface_closed_cb(self, s):
        ...
        import Surface
        Surface.set_coloring_method('msms', self, None)
        Surface.set_visibility_method('msms', self, None)
...
# -----------------------------------------------------------------------------
#
def atom_surface_rgba(a):

    c = a.surfaceColor or a.molecule.color
    rgba = c.rgba()
    o = a.surfaceOpacity
    if o >= 0:
        rgba = rgba[:3] + (o,)
