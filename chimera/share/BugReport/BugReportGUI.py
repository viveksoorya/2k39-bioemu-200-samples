...
import BugReport
BUG_HOST = "www.cgl.ucsf.edu"
BUG_SELECTOR = "/cgi-bin/chimera_bug_report_2.py"
...
class BugReportGUI(WizShell):
	...
	if not self.checkConnection():
		...
		"The UCSF Computer Graphics Lab web server is unreachable!</font>"
		"<p>Thank you for assisting us in the development process"
		" by using our feedback submission system."
		"  It appears that either your computer is not connected"
		" to the Internet"
		" or that the Internet connectivity to UCSF is broken"
		" or that the UCSF Computer Graphics Lab web server is down."
		"  An Internet connection is needed in order to send the"
		" feedback to the UCSF Computer Graphics Lab."
		"  Please try again when your computer is connected to the"
		" Internet.</p>"
		"<p>If you <i>are</i> connected to the Internet,"
		" but use a proxy to connect to web servers,"
		" you can try configuring the proxy settings in the"
		" <a href=\"python:import dialogs, DBPuppet;dialogs.display('preferences')."
		"setCategoryMenu(DBPuppet.waprefs.WEBACCESS_PREF)\" "
	...
	self.createExplanation(1,
		...
		" occurred and an email address that can be used to contact"
		" you for more information.", height=5, width=60)
	...
	self.include_reply_log = Tkinter.IntVar(self.entryFrame)
	self.include_reply_log.set(True)
	Tkinter.Checkbutton(self.entryFrame, variable=self.include_reply_log,
		text="Include reply log contents in bug report").grid(
	...
	if not email and ((not desc) or (BugReport.ADDL_INFO in desc)):
		...
		def __init__(self):
			...
			"were doing when the bug occurred.", icon='info')
		BacktoForm = AskYesNoDialog.Yes
		SubmitAnyway = AskYesNoDialog.No
	    if MissingEmail().run(self._toplevel) != 'no':
		return False
	return True
	...
	# include replr log contents in description
	if self.include_reply_log.get():
		entry_values['description'] += "\n\nReply log:\n"
	...
	from CGLutil.multipart import post_multipart_formdata
