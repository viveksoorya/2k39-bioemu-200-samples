...
class Surface_Capper:
    ...
    def __init__(self):
        ...
        self.trigger_handlers = {
            ...
            ('Viewer', self.viewer_changed_cb): None,
            ('OpenState', self.xform_changed_cb): None,
            ('SurfacePiece', self.surface_piece_changed_cb): None,
            }
    ...
    def model_changed_cb(self, trigName, myData, trigData):
        ...
        if ('useClipPlane changed' in r or
            ...
            models = set(trigData.modified)
            models.update(trigData.created)
            mlist = [m for m in self.surface_models() if m in models]
    ...
    # -------------------------------------------------------------------------
    # Capture turning near/far clip planes on and off.
    #
    def viewer_changed_cb(self, trigName, myData, trigData):

        if 'clipping changed' in trigData.reasons:
            self.update_caps(self.surface_models())
    ...
    # -------------------------------------------------------------------------
    # Color or display mask for surface piece changed.
    #
    def surface_piece_changed_cb(self, trigName, myData, trigData):

        r = trigData.reasons
        recolor = ('color changed' in r and self.cap_color is None)
        recalc = ('display mask changed' in r or recolor)
        if recolor or recalc:
            for p in trigData.modified:
                if not p.__destroyed__ and not is_surface_cap(p):
                    mc = model_capper(p.model, create = False)
                    if mc:
                        if recalc:
                            mc.update_piece_caps(p)
                        if recolor and not p.hasVertexColors():
                            for cp in mc.cap_pieces():
                                cp.color = p.color
...
class Model_Capper:
    ...
    def update_cap_properties(self, cp, displayed, mesh_style, color):
        ...
        cp.displayStyle = cp.Mesh if mesh_style else cp.Solid
    ...
    def cap_surface_piece(self, p, cap_name, plane_normal, plane_offset,
        ...
        if self.has_cap_geometry_changed(cp, plane_normal, plane_offset,
            ...
            cp = self.update_cap_geometry(p, cap_name,
                                          ...
                                          subdivision_factor)
        if cp:
            c = color if p.hasVertexColors() else (color or p.color)
            self.update_cap_properties(cp, p.display, mesh_style, c)
...
def cap_color(p):
    ...
    m = p.model
    import chimera
    if isinstance(m, chimera.MSMSModel):
        mol = m.molecule
        if mol and not mol.__destroyed__:
            return mol.color.rgba()
    return p.color
...
def capped_surfaces():
    ...
    from chimera import openModels as om
    from _surface import SurfaceModel
    surfs = [s for s in om.list(modelTypes = [SurfaceModel]) if model_capper(s)]
    return surfs
...
def save_session_cb(trigger, x, file):
    ...
    import session
    session.save_capper_state(file)

# -----------------------------------------------------------------------------
#
def register_session_saving():
    
    import SimpleSession
    from chimera import triggers
    triggers.addHandler(SimpleSession.SAVE_SESSION, save_session_cb, None)

# -----------------------------------------------------------------------------
#
the_capper = None
def capper(create = True):

    global the_capper
    if the_capper is None and create:
        the_capper = Surface_Capper()
        register_session_saving()
    return the_capper
