...
# --- UCSF Chimera Copyright ---
#
# $Id: __init__.py 41992 2019-01-16 21:38:29Z pett $
...
def addAttributes(attrFile, models=None, log=False, raiseTool=True):
...
def _addAttr(attrFile, models, log, raiseTool):
	setAttrs = {}
	colors = {}

	recipMapping = {
		# relevant attrs of ItemizedSelection,
		# since that's what evalSpec() returns
		"molecules": chimera.Molecule,
		"chains": chimera.Sequence.Sequence,
		"residues": chimera.Residue,
		"bonds": chimera.Bond,
		"pseudobonds": chimera.PseudoBond,
		"atoms": chimera.Atom
	}
	...
	for rawControl, data in readDataFile(attrFile):
		...
		legalNames = ["attribute", "match mode", "recipient", "none handling"]
		...
		noneHandlings = ["string", "None", "delete"]
		noneHandling = control["none handling"]
		if noneHandling not in noneHandlings:
			raise SyntaxError("Unknown match mode (%s) specified.\n"
				"It must be one of: %s" % (
				control["none handling"], ", ".join(noneHandlings)))
		if noneHandling == "string":
			noneOkay = False
		else:
			noneOkay = True

		recipient = control["recipient"]
		if recipient not in recipMapping:
			raise SyntaxError("Unknown attribute recipient (%s)"
				" specified.\nMust be one of: %s" % (recipient,
				", ".join(recipMapping.keys())))
		preExistingAttr = getattr(recipMapping[recipient], attrName, None)
		if preExistingAttr and callable(preExistingAttr):
			raise ValueError("%s is a method of the %s class and cannot be"
				" redefined" % (attrName, recipMapping[recipient].__name__))
		if attrName[0].isupper() and preExistingAttr:
			raise ValueError("%s is a constant in the %s class and cannot be"
				" redefined" % (attrName, recipMapping[recipient].__name__))
		dataErrors = []
		for lineNum, d in enumerate(data):
			...
			if colorAttr:
				...
				value = convertType(value, noneOkay=noneOkay)
			for match in matches:
				if value != None or noneHandling == "None":
					setattr(match, attrName, value)
				elif hasattr(match, attrName):
					delattr(match, attrName)
			if matches and not colorAttr and value is not None:
	...
	if setAttrs and not chimera.nogui and raiseTool:
		from ShowAttr import ShowAttrDialog, objectTypes
		from chimera import dialogs
		showableAttr = False
		reasons = []
		for recipient, attrName in setAttrs.keys():
			validRecipients = ["atoms", "residues", "molecules"]
			if recipient not in validRecipients:
				reasons.append("%s not assigned to atoms,"
					" residues, or molecules" % attrName)
				continue
			attrObj = objectTypes[validRecipients.index(recipient)]
			if attrName in attrObj.screenedAttrs:
