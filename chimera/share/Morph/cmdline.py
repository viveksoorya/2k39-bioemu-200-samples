...
def startCommand(molecule, name="default", method="corkscrew",
			rate="linear", frames=20, cartesian=False, core=0.5):
	from chimera import UserError
	if len(molecule) != 1:
...
def interpolateCommand(molecule, name="default", method=None, rate=None,
			frames=None, cartesian=False, core=0.5):
	if len(molecule) != 1:
		from chimera import UserError
		raise UserError("You must specify exactly one molecule")
	m = molecule[0]
...
def movieCommand(name="default", minimize=None, steps=None, nogui=False):
	import util
	ms = _find(name)
	ms.updateMovieDialog(True, minimize=minimize, steps=steps, nogui=nogui)
