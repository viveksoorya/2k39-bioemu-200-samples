class MolecularMotion:
	...
	def __init__(self, m, xform, **kw):
		...
		Currently support keyword options are:
			...
			core		real, default 0.5
					Fraction of model to be considered
					"core" and move as a unit
	...
	def _resetOptions(self, **kw):
		...
		self.coreFraction = kw.get("core", 0.5)
	...
	def interpolate(self, m, xform, **kw):
		...
		#
		# If the given model has a different color than
		# previous models, then set new model color to None
		#
		if self.mol.color != m.color:
			self.mol.color = None
		...
		sm = self.mol
		try:
			results = Segment.segmentHingeExact(sm, m, self.coreFraction)
		except ValueError:
			results = Segment.segmentHingeApproximate(sm, m, self.coreFraction)
