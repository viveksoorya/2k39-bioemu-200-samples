...
def copyMolecule(m, copyXformCoords=False, copyPBG=True):
	...
	if copyPBG:
		coordinationBondGroup = m.metalComplexGroup(create=False)
		if coordinationBondGroup:
			cbg = cm.metalComplexGroup()
			for pb in coordinationBondGroup.pseudoBonds:
				try:
					atoms = [ atomMap[a] for a in pb.atoms ]
				except KeyError:
					# Probably bond to non-primary atom
					pass
				else:
					cbg.newPseudoBond(*atoms)
...
def mapResidues(m0, m1):
	if len(m0.residues) != len(m1.residues):
		raise ValueError("molecules have different # residues")
	residueMap = {}
