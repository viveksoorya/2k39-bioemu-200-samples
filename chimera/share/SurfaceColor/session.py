    ...
    for surface_id_subid, (color_state, caps_only) in self.coloring_table.items():
      ...
      if len(slist) > 0:
        ...
        if color_source is None:
          ...
          replyobj.warning('Could not restore surface color on surface model\nwith id %d.%d because volume used in coloring was not restored.\n' % surface_id_subid)
          return
...
class Base_Volume_Color_State:
  ...
  def state_from_color_source(self, color_source):
    ...
    from VolumeViewer.session import session_volume_id
    self.session_volume_id = session_volume_id(color_source.volume)
