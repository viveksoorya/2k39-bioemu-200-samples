...
class Surface_Color_Dialog(ModelessDialog):
  ...
  def fillInUI(self, parent):
    ...
    cp = Hybrid.Option_Menu(cf, 'Palette', *tuple(gui_palette_names.keys()))
    ...
    co = Hybrid.Checkbutton(opf, 'Only color sliced surface face', False)
    co.button.grid(row = orow, column = 0, sticky = 'w')
    orow += 1
    self.color_caps_only = co.variable

    sv = Hybrid.Checkbutton(opf, 'Report value at mouse position', False)
    sv.button.grid(row = orow, column = 0, sticky = 'w')
    orow += 1
    self.report_values = sv.variable
    sv.callback(self.report_values_cb)
  ...
  def change_color_palette_cb(self):
    ...
    p = self.palette.get()
    from SurfaceColor import gui_palette_names
    self.colormap_gui.set_color_palette(gui_palette_names[p])
  ...
  # ---------------------------------------------------------------------------
  #
  def report_values_cb(self):

    if self.report_values.get() and self.tracking_mouse is None:
      from chimera.tkgui import app
      h = app.graphics.bind('<Any-Motion>', self.mouse_motion_cb, add=True)
      self.tracking_mouse = h

  # ---------------------------------------------------------------------------
  #
  def mouse_motion_cb(self, event):

    if self.report_values.get() and self.isVisible():
      import SurfaceColor as SC
      vpn = SC.surface_value_at_window_position(event.x, event.y)
      if not vpn is None:
        v, p, n = vpn
        from chimera.replyobj import status
        status('%s %-#8.5g  x %-#8.5g  y %-#8.5g  z %-#8.5g'
               % ((n, v) + tuple(p)))
