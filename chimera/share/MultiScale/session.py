...
# -----------------------------------------------------------------------------
#
def save_multiscale_state(multiscale_manager, multiscale_dialog, file):

  s = Multiscale_Dialog_State()
  s.state_from_dialog(multiscale_manager, multiscale_dialog)
...
# -----------------------------------------------------------------------------
#
def restore_multiscale_state(multiscale_state):
  ...
  from SessionUtil import objecttree
  s = objecttree.basic_tree_to_instance_tree(multiscale_state, name_to_class)

  import MultiScale
  mm = MultiScale.multiscale_manager()
  d = MultiScale.multiscale_model_dialog(create = s.dialog_exists())
  s.restore_state(mm, d)
...
class Multiscale_Dialog_State:
  ...
  # ---------------------------------------------------------------------------
  #
  def state_from_dialog(self, multiscale_manager, multiscale_dialog):
    ...
    lmlist = self.lan_molecule_list(mm)
    self.lan_molecule_states = self.lan_molecule_states(lmlist)
    self.model_states = model_piece_states(mm.models)

  # ---------------------------------------------------------------------------
  #
  def lan_molecule_list(self, multiscale_manager):

    # Get list of all LAN_Molecule objects.
    lmtable = {}
    for cp in multiscale_manager.chain_pieces():
  ...
  def dialog_exists(self):
    ...
    return hasattr(self, 'is_visible')

  # ---------------------------------------------------------------------------
  #
  def restore_state(self, multiscale_manager, multiscale_dialog):

    d = multiscale_dialog
    if d and self.dialog_exists():
      if self.is_visible:
        d.enter()
    ...
    self.restore_multiscale_models(multiscale_manager, lmlist)
  
  # ---------------------------------------------------------------------------
  #
  def restore_multiscale_models(self, multiscale_manager, lmlist):
  ...
  def restore_chain_surfaces(self, chain_pieces):
    ...
    for cp in chain_pieces:
      if hasattr(cp, 'session_restore_surface_parameters'):
        ...
        if not surf_params[0] is None:
          # Have to show surface so surface_piece is made.
...
class Chain_Piece_State:
  ...
  def create_object(self, lan_chain_table, surfacer):
      ...
      # Need to fix multiscale_chain_data cached atom positions for the
      # source_molecule to use the atom positions before the xform was
      # applied to atom coordinates.  This is ugly voodoo.  Usually the
      # source molecule has the identity transform so it is not needed.
      # Must do this before creating Chain_Piece since that sets atom positions
      # using the cached source atom positions.
      lm = lan_chain.lan_molecule
      if lm.molecule(load = False) is lm.source_molecule:
        import Matrix
        Matrix.xform_points(lan_chain.source_atom_xyz(), xform.inverse())
