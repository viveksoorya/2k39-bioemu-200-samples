...
def register_accelerators():
  ...
  accelerators = [
    ('3d', 'Switch between sequential stereo and mono', toggle_stereo),
    ...
    ('ri', 'Select residue interval', sel_residue_interval),
    ('rl', 'Reply log', show_reply_log),
    ('rn', 'Select next residue', sel_next_residue),
    ('rp', 'Select previous residue', sel_previous_residue),
    ('rr', 'Round ribbon', round_ribbon),
    ('rx', 'Show rapid access panel', rapid_access),
    ...
    ('sh', 'Toggle shadows', toggle_shadows),
...
#
def toggle_stereo():
  'Toggle between sequential stereo and mono camera modes.'
  from chimera import viewing, tkgui
  viewer = tkgui.app.viewer
  mode = 'sequential stereo' if viewer.camera.mode() == 'mono' else 'mono'
  viewing.setCameraMode(mode, viewer)
...
def clip_selected_models():
  ...
  if cm is None or not cm.useClipPlane:
    ...
    cd = dialogs.find(ModelClip.gui.ClipDialog.name)
    if cd:
      cd.stopMouseClip()
...
def clip_dialog():
  ...
  cd = dialogs.find(ModelClip.gui.ClipDialog.name)
  if cd is None:
    cd = dialogs.find(ModelClip.gui.ClipDialog.name, create = True)
    cd.Close()
  return cd
...
def show_per_model_clipping_dialog():
  ...
  from chimera import dialogs
  import ModelClip.gui
  dialogs.display(ModelClip.gui.ClipDialog.name)
...
#
def rapid_access():
  'Toggle display of rapid access panel'
  from chimera.tkgui import app
  ra = app.rapidAccess
  ra.shown = not ra.shown
...
#
def toggle_shadows():
  'Toggle display of interactive shadows'
  import chimera
  v = chimera.viewer
  try:
    v.showShadows = not v.showShadows
  except chimera.error:
    from chimera import replyobj
    replyobj.status('Shadows not supported by graphics driver.')
...
def set_background(rgb):
  from chimera import tkgui, MaterialColor
  color = MaterialColor(*rgb)
  tkgui.app.viewer.background = color
...
# -----------------------------------------------------------------------------
#
def sel_next_residue():
  from chimera import selection
  import Molecule
  Molecule.select_next_residue(selection.currentResidues())

# -----------------------------------------------------------------------------
#
def sel_previous_residue():
  from chimera import selection
  import Molecule
  Molecule.select_next_residue(selection.currentResidues(), backwards = True)

# -----------------------------------------------------------------------------
#
def sel_residue_interval():
  from chimera import selection
  import Molecule
  Molecule.select_residue_interval(selection.currentResidues())
