...
class Volume_Manager:
  ...
  def open_models_cb(self, trigger_name, args, models):
    ...
    vlist = [v for v in models if isinstance(v, Volume)]
    if vlist:
      self.add_volumes(vlist)
      
  # ---------------------------------------------------------------------------
  #
  def add_volumes(self, vlist):

    self.data_regions.extend(vlist)
...
class Volume(SurfaceModel):
  ...
  def __init__(self, data, region = None, rendering_options = None,
    ...
    from chimera import Material
    self.material = Material.lookup("default")
  ...
  def call_change_callbacks(self, change_types):
    ...
    if isinstance(change_types, basestring):
  ...
  def new_region(self, ijk_min = None, ijk_max = None, ijk_step = None,
    ...
    if same_region(region, self.region):
      return False
  ...
  def initialize_thresholds(self, first_time_only = True,
    ...
    if s.has_nan:
      ...
      count = isnan(self.matrix()).sum()
      from chimera.replyobj import error
      error('Map %s contains %d NaN (not a number) values.  ' % (self.name, count) +
            'This will cause errors in Chimera because it cannot handle NaN values.')
  ...
  def show(self, representation = None, rendering_options = None, show = True):
    ...
    if show:
      ...
      from chimera import CancelOperation
      try:
        self.keep_displayed_data = self.displayed_matrices(require_levels = True)
      except CancelOperation:
        return
    else:
  ...
  def update_surface_piece(self, level, rgba, show_mesh, rendering_options,
    ...
    previous_color = p.color
    p.color = rgba
    if p.color != previous_color and p.hasVertexColors():
      # Turn off per-vertex coloring because new single color requested
      p.vertexColors = None
      import Surface
      Surface.set_coloring_method('single color', self)
    ...
    elif show_mesh: lit = (ro.mesh_lighting or rgba[3] < 1)
    else:           lit = True
    p.useLighting = lit
  ...
  def surface_level_for_enclosed_volume(self, volume, tolerance = 1e-3,
    ...
    level = VD.surface_level_enclosing_volume(matrix, gvolume, tolerance, max_bisections)
    return level
  ...
  # ---------------------------------------------------------------------------
  #
  def update_solid(self, rendering_options, show = True):
  ...
  def copy_settings_from(self, v,
    ...
    if copy_vertex_colors:
      sp, vp = self.surfacePieces, v.surfacePieces
      if len(sp) == len(vp):
        for p0,p1 in zip(vp,sp):
          if p0.hasVertexColors() and p0.vertexCount == p1.vertexCount:
            p1.vertexColors = p0.vertexColors
    ...
    if copy_clipping:
      sm = v.surface_model()
      sm_copy = self.surface_model()
      if sm.useClipPlane or sm_copy.useClipPlane:
        sm_copy.useClipPlane = sm.useClipPlane
        sm_copy.clipPlane = sm.clipPlane
        sm_copy.clipThickness = sm.clipThickness

    if copy_hide_dust:
      sm = v.surface_model()
      sm_copy = self.surface_model()
      from HideDust import dust
      hd = dust.hiding_dust(sm)
      if hd or dust.hiding_dust(sm_copy):
        if hd:
          metric, limit = dust.dust_limit(sm)
          dust.hide_dust(sm_copy, metric, limit, True)
        else:
          dust.unhide_dust(sm_copy)
  ...
  def bounding_region(self, points, padding = 0, step = None, clamp = True, cubify = False):
    ...
    ijk_fmin, ijk_fmax = points_ijk_bounds(points, padding, d)	# Float values
  ...
  def ijk_to_global_xyz(self, ijk):
    ...
    x,y,z = self.data.ijk_to_xyz(ijk)
    xf = self.model_transform()
    from chimera import Point
    gxyz = xf.apply(Point(x,y,z)).data()
    return gxyz
  ...
  def axis_vector(self, axis):
    ...
    vn = Vector(*tuple(array(d.ijk_to_xyz(va)) - d.ijk_to_xyz((0,0,0))))
    n = self.openState.xform.apply(vn)
    n.normalize()
    return n.data()
  ...
  def matrix_plane(self, axis, mplane, read_matrix = True):
    ...
    if not read_matrix and m is None:
      return None
  ...
  def subregion(self, step = None, subregion = None):
    ...
    r = (tuple(ijk_min), tuple(ijk_max), tuple(ijk_step))
    return r
  ...
  # ---------------------------------------------------------------------------
  # Returns the matrices even if the map is not currently displayed.
  # If require_levels is true then surface style must have at least one contour
  # level and solid style at least one transfer function level or no matrices
  # are returned.
  #
  def displayed_matrices(self, read_matrix = True, require_levels = False):

    matrices = []

    solid = (self.representation == 'solid')
    if require_levels:
      if ((solid and len(self.solid_levels) == 0) or
          (not solid and len(self.surface_levels) == 0)):
        return matrices # No levels shown
...
def same_grid(v1, region1, v2, region2):
  ...
  same = (same_region(region1, region2) and
...
def full_region(size, ijk_step = (1,1,1)):
  ...
  ijk_min = (0, 0, 0)
  ijk_max = tuple(n-1 for n in size)
  region = (ijk_min, ijk_max, ijk_step)
  return region
...
def open_volume_file(path, format = None, name = None, representation = None,
                     ...
                     show_data = True, show_dialog = True, polar_values = False):
  ...
  groups = [g for g in glist if isinstance(g,list)]
  if groups:
    # Handle multi-channel times series, assigning one model id to each series.  Ick.
    from chimera import openModels
    for gr in groups:
      vlist = open_volumes_from_grids(gr, name, representation, False,
                                      model_id, show_data, show_dialog)
      openModels.add(vlist)
    glist = [g for g in glist if not isinstance(g,list)]
    
  vlist = open_volumes_from_grids(glist, name, representation, open_models,
                                  model_id, show_data, show_dialog)
  return vlist

# -----------------------------------------------------------------------------
#
def open_volumes_from_grids(glist, name, representation, open_models,
                            model_id, show_data, show_dialog):
  ...
  vlist = [volume_from_grid_data(g, representation, open_models, model_id,
                                 (show_data and (not hasattr(g, 'series_index') or g.series_index == 0)),
                                 show_dialog)
...
def volume_from_grid_data(grid_data, representation = None,
  ...
  if one_plane:
    k = grid_data.size[2]/2
    (i0,j0,k0),(i1,j1,k1) = region
    region = (i0,j0,k),(i1,j1,k)
