...
class Volume_Dialog(ModelessDialog):
  ...
  def fillInUI(self, parent):
    ...
    self.volume_opened_cb(volume_list())
  ...
  def volume_closed_cb(self, vlist):
    ...
    # Update focus region.
    if self.focus_region in vlist:
      self.focus_region = None
      vset = set(vlist)
      import volume
      vopen = [v for v in volume.volume_list() if v.data and not v in vset]
...
class Thresholds_Panel(Hybrid.Popup_Panel):
  ...
  def __init__(self, dialog, parent):
    ...
    self.value_type_popup = Hybrid.Balloon_Help(rh, 'value type', delay = 0.5)
  ...
  def update_data_range(self, volume = None, delay = 0.5):
    ...
    self.value_type_popup.text = 'value type %s' % str(volume.data.value_type)
...
class Histogram_Pane:
  ...
  def plot_surface_levels(self):
    ...
    surf_markers = [Marker((t,0), c) for t,c in zip(dr.surface_levels, dr.surface_colors)]
  ...
  def plot_solid_levels(self):
    ...
    from histogram import Marker
    solid_markers = [Marker(ts, c) for ts, c in zip(dr.solid_levels, dr.solid_colors)]
...
#
class Orthoplane_Panel(Hybrid.Popup_Panel):
