...
class Solid:
  ...
  def __init__(self, name, size, value_type, matrix_id, matrix_plane, transform,
    ...
    from chimera import Model
    self.attached_model = align if isinstance(align, Model) else None
  ...
  # ---------------------------------------------------------------------------
  #
  def update_model(self, open = True, show = True):
