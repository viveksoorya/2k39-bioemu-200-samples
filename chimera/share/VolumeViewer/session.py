  ...
  def restore_state(self, volume_dialog):
    ...
    if self.version >= 10:
      ...
      if drlist != cur_drlist:
        map(tp.close_histogram_pane, tuple(tp.histogram_panes))
        map(tp.update_panel_widgets, drlist)
    ...
    if self.version >= 2 and (path, self.grid_id) in gdcache:
      ...
      if self.version >= 2:
        ...
        if not id in gdcache:
          from chimera import replyobj
          replyobj.info('Could not find %s, grid id %s' % id)
          return []
        dlist = [gdcache[id]]
      else:
...
class Volume_State:
  ...
  def create_object(self, data):
    ...
    if self.version >= 3:
      ...
      for i,d in enumerate(data):
        ...
        if self.name:
          ...
          sid = v.name
        v.session_volume_id = sid
    if isinstance(v.data.path, basestring):
    ...
    for v in vlist:
      if ((sm and v.representation in ('surface', 'mesh')) or
          (som and v.representation == 'solid')):
        v.show()
      if sm:
