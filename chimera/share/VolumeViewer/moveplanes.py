...
class Planes_Mouse_Mode:
    ...
    def mouse_down(self, viewer, event):
        ...
        self.map, self.axis, self.side, self.ijk = nearest_volume_face(x,y)
        self.drag = False
    ...
    def mouse_drag(self, viewer, event):
        ...
        if _frameNumber == self.frame_number:
            # Avoid slow interaction caused by reading data planes that are
            # never displayed.
            return
        ...
        step = speed * drag_distance(v, self.ijk, self.axis, dx, dy)
...
def drag_distance(v, ijk, axis, dx, dy, clamp_speed = 3):
    from math import sqrt
    d = sqrt(dx*dx + dy*dy)
    nx,ny,nz = v.axis_vector(axis)    # global coords
    ...
    import chimera
    psize = chimera.viewer.camera.pixelSize(z)
