...
# --- UCSF Chimera Copyright ---
#
# $Id: miller.py 42505 2023-11-27 21:28:19Z pett $
...
# miller browser columns cause a lot of stack usage on Aqua...
def doubleAquaStack():
	from chimera.tkgui import windowSystem
	if windowSystem == 'aqua':
		sys.setrecursionlimit(2 * sys.getrecursionlimit())
chimera.registerPostGraphicsFunc(doubleAquaStack)
...
class MillerBrowser(Tkinter.Frame):
	...
	def __init__(self, parent=None, style='open', multiple=0,
		...
		if self.style == 'save':
			sfKw = {'clipper_height': '2.5i'}
		else:
			sfKw = {}
		prefName = "preferred" + self.style.capitalize()
		prefWidth = _prefs[prefName + "Width"]
		prefHeight = _prefs[prefName + "Height"]
		if prefWidth is not None:
			sfKw['clipper_width'] = prefWidth
			sfKw['clipper_height'] = prefHeight
		self.scrolledFrame = Pmw.ScrolledFrame(self, vertflex='elastic', **sfKw)
		self._lastFrameWidth = None
		self.scrolledFrame.interior().bind('<Configure>', self.columnChange, add=1)
		clip = self.scrolledFrame.component('clipper')
		self._prefSaveWaitID = None
		def reportResize(event, clip=clip, prefName=prefName):
			width = int(clip.winfo_width())
			height = int(clip.winfo_height())
			if width > 10 and height > 10:
				if self._prefSaveWaitID:
					self.after_cancel(self._prevSaveWaitID)
				self._savedWH = (width, height)
				# resizes are _slow_ if pref-save not delayed
				self._prevSaveWaitID = self.after(3000, self._savePrefWH)
		# since wm_sizefrom doesn't actually return anything useful, do some
		# fancy footwork to try to just get user-driven configures...
		def delayBind(clip=clip):
			clip.after(500, lambda clip=clip: clip.bind('<Configure>', reportResize, add=1))
		clip.after_idle(delayBind)

		if initialdir is None:
			for faveDir in rememberer.faveDirs():
		...
		if self.pathExists(initialdir):
			try:
				...
				cwd = os.getcwd()
				if isinstance(cwd, unicode) == isinstance(initialdir, unicode):
					inCommon = os.path.commonprefix([initialdir, cwd])
				else:
					# some bad character in one or the other
					inCommon = None
				if not inCommon:
					inCommon = cwd
					errorPart = None
...
it first via Network Neighborhood and try again\n""" % unc)
			...
			errorPart = None
		prevPath, prob = os.path.split(self.mainColumn.getPaths()[0])
		if not prob:
			prevPath, prob = os.path.split(prevPath)
		self.mainColumn.remove()
		try:
			self.makeMainColumn(path, errorPath=errorPart)
		except IOError, v:
			self.makeMainColumn(prevPath)
	...
	def setFilter(self, descript, filt=None):
		...
		rememberer.rememberFilter(descript, self)
	...
	def replaceFilters(self, filters, defaultFilter=None):
		if self.style == 'save':
			self.saveExtension.clear()
			fl = []
			for d, globs, ext in filters:
				self.saveExtension[d] = ext
				fl.append((d, globs))
			filters = fl
		if not hasattr(self, 'filters'):
		...
		self.makeFilterMenu()
		initialFilter = rememberer.lastFilter(self)
		if initialFilter:
	...
	def makeFaveDirs(self, gotoDir=None):
		self.dirFaves.clear()
		self.dirFaves.setlist(rememberer.faveDirs())
		curDir = self.mainColumn.getDirPath()
		if gotoDir == curDir:
			self._setDirEntry(curDir)
		else:
			self.setPath(gotoDir)

	def makeFaveFiles(self):
		curFave = self.fileFaves.get()
		self.fileFaves.clear()
		self.fileFaves.setlist(rememberer.faveFiles(self))
	...
	def _savePrefWH(self, *args):
		self._prevSaveWaitID = None
		(width, height) = self._savedWH
		prefName = "preferred" + self.style.capitalize()
		_prefs[prefName + "Width"] = width
		_prefs[prefName + "Height"] = height
...
class MillerColumn(Pmw.ScrolledListBox):
	...
	def revive(self, head):
		...
		self.setlist(items)
		self.component('listbox').configure(width=_colWidth(items))
	...
	def newFolder(self, folderName):
		...
		if sys.platform == "win32":
			# Windows does not allow trailing spaces in folder names (#19823)
		...
		if os.sep in folderName:
			raise UserError("New folder name cannot contain '%s' character" % os.sep)
...
""" % path)
	...
	def _refresh(self):
		...
		if self.filt:
			...
			items = dirs + files
		filt_items = filter(lambda i: not i.startswith(".") or i in cursel,
									items)
		items = []
		for i in filt_items:
			if tcl_okay(i):
...
defaults = {"dirHistory": [], "fileHistory": {}, "lastFilter": {},
	"preferredOpenWidth": None, "preferredSaveWidth": None,
	"preferredOpenHeight": None, "preferredSaveHeight": None}
from copy import deepcopy
_prefs = preferences.addCategory("miller browser", preferences.HiddenCategory,
		optDict=deepcopy(defaults))
...
class _Rememberer:
	...
	def _remember(self, name, place="dirs", raiseUp=1, instance=None):
		...
		if place == "dirs":
			...
			from copy import deepcopy
			histCopy = deepcopy(_prefs["fileHistory"])
			histCopy[instance.historyID] = newHist
			_prefs["fileHistory"] = histCopy
	...
	def rememberDir(self, name):
		...
		gotoDir = name[-1] if isinstance(name, list) else name
		if not os.path.isdir(gotoDir):
			gotoDir, tail = os.path.split(gotoDir)
		for inst in self._instances:
			inst.makeFaveDirs(gotoDir=gotoDir)
	...
	def rememberFilter(self, filt, instance):
		if instance.historyID:
			from copy import deepcopy
			filtCopy = deepcopy(_prefs["lastFilter"])
			filtCopy[instance.historyID] = filt
			_prefs["lastFilter"] = filtCopy

	def faveDirs(self):
		return [d for d in _prefs["dirHistory"] if tcl_okay(d)]

	def faveFiles(self, instance):
		try:
			return [f for f in _prefs["fileHistory"][instance.historyID] if tcl_okay(f)]
		except KeyError:
			return []
...
def listDir(path):
	...
	if platform.platform().startswith("Windows-7-"):
		def isFolderShortcut(folder, entry):
			path = os.path.join(folder, entry)
			if not os.path.isdir(path):
				return False
			import stat
			pathStat = os.stat(path)
			size, mode = pathStat.st_size, pathStat.st_mode
			if size == 0 and (mode & stat.S_IWUSR) and (
					mode & stat.S_IWGRP) and (mode & stat.S_IWOTH):
				# folders on network/USB drives frequently have these
				# characteristics, so also check that listdir fails
				try:
					os.listdir(path)
				except:
					return True
	...
	return [i for i in os.listdir(unicode(path))
		if isinstance(i, unicode) and not isFolderShortcut(path, i)]

rememberer = _Rememberer() # needs to be after handleAccented

def _colWidth(items):
	return min(100, max([len(i) for i in items] + [20]))

def tcl_okay(item):
	for c in item:
		if 0xD800 <= ord(c) <= 0xDBFF or ord(c) > 0xFFFF:
			# extended Unicode; Tcl cannot handle such characters
			return False
	return True
