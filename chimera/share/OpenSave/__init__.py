...
# --- UCSF Chimera Copyright ---
#
# $Id: __init__.py 42457 2023-05-11 23:35:26Z pett $
...
"""Standard open/save panels for Chimera
   ...
   In save dialogs, the 'append' keyword indicates that the file will be
   appended to rather than overwritten.  This information is used in
   the confirmation dialog displayed to the user.
...
class OpenSaveBase:
	...
	def __init__(self, clientPos=None, clientSticky=None, command=None,
		...
		self.triggers.addTrigger(self.PATHS_CHANGED)
	...
	def fillInUI(self, parent):
		...
		self.replaceFilters = self.millerBrowser.replaceFilters
	...
	def _millerReady(self, paths):
		if paths:
			...
			state = 'disabled'
		b = self.buttonWidgets[self.default]
		b.config(state=state)
		if not self._positioned:
			self.initialPosition()
			self._positioned = True
		self.triggers.activateTrigger(self.PATHS_CHANGED, paths)

class ModalBase(ModalDialog):
	dismiss = 'Cancel'
	def __init__(self, *args, **kw):
		kw['initiateAutoPositioning'] = False
		ModalDialog.__init__(self, *args, **kw)

	def OK(self):
		if self.millerKw.get('filters', None):
			self.returnValue = self.millerBrowser.getPathsAndTypes()
		else:
			self.returnValue = [(p, None) for p in self.millerBrowser.getPaths()]
...
class SaveBase(OpenSaveBase):
	...
	def __init__(self, **kw):
		# avoid overridding subclass-defined default...
		self.__append = kw.pop('append', False)
		if 'default' in kw:
			self.default = kw.pop('default')
			# also, if using keyword default presumably
	...
	def Save(self):
		...
		if preferences.get(GENERAL, CONFIRM_OVERWRITE):
			...
			if windowSystem == 'aqua':
				# stupid workaround that prevents
				# the modal dialog from not showing up
				# if 'Save' comes from pressing Return
				# in entry field rather than from
			...
			for path in self.millerBrowser.getPaths():
				...
				if (os.path.exists(path) and
					...
					if self.__append:
						...
						verb = "Overwrite"
					dlg = AskYesNoDialog("%s %s?" % (verb, path))
