...
# --- UCSF Chimera Copyright ---
#
# $Id: gui.py 41998 2019-01-25 18:21:36Z pett $
...
class CombineDialog(ModelessDialog):
	...
	def Apply(self):
		...
		closeSourceModels = self.closeModelsVar.get()
		mid = self.modelID.get()
		openMols = [m for m in openModels.list()
			if not closeSourceModels or m not in mols]
		if mid in set([m.id for m in openMols if m.subid == 0]):
			self.enter()
			raise UserError("Must specify model ID"
		...
		openModels.add([m], baseId=mid, shareXform=False)
		m.openState.xform = refMol.openState.xform
		restoreNewMoleculeProcessing()

		if closeSourceModels:
			openModels.close(mols)

		self.modelID.set(self._firstAvailableID())

	def _firstAvailableID(self):
