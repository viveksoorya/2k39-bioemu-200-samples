...
# --- UCSF Chimera Copyright ---
#
# $Id: __init__.py 42018 2019-04-05 19:45:50Z pett $
...
def combine(mols, refMol, newChainIDs=True, log=False, returnMapping=False):
	...
	if newChainIDs:
		if numChains > 63:
			raise CombineError("More than 63 chains; cannot"
		...
		chainIDmap = {}
		# try to keep the original order when renaming chains 
		for chainID, idMols in sorted(origChainIDs.items()):
			if len(chainID) > 1:
				continue
			if chainID == ' ' and len(idMols) > 1:
				# if multiple chain IDs are blank, remap _all_ the chains
				remapStart = 0
				usedChainIDs.remove(' ')
			else:
	...
	combined.lowerCaseChains = False
	...
	resAttrs = optionalAttributes[Residue].keys() + ['ribbonColor',
		'labelColor', 'isHelix', 'isSheet', 'ribbonDrawMode',
		'ribbonDisplay', 'label', 'isHet']
	serial = 1
	atomMap = {}
	globalHelixOffset = globalStrandOffset = 0
	for m in mols:
		if m.openState == refMol.openState:
			...
			xform.invert()
		localHelixMax = localStrandMax = 0
		for r in m.residues:
			chainID, pos = resMap[r]
			if chainID.islower():
				combined.lowerCaseChains = True
			...
			for attrName in resAttrs:
				try:
					...
					continue
			if nr.isHelix:
				nr.ssId = r.ssId + globalHelixOffset
				localHelixMax = max(r.ssId, localHelixMax)
			elif nr.isStrand:
				nr.ssId = r.ssId + globalStrandOffset
				localStrandMax = max(r.ssId, localStrandMax)
			else:
				nr.ssId = r.ssId
			ratoms = r.atoms
			...
			for a in ratoms:
				...
				na = addAtom(a.name, a.element, nr, crd,
							serialNumber=serial)
				na.altLoc = a.altLoc
				if a.idatmIsExplicit:
					na.idatmType = a.idatmType
				atomMap[a] = na
				serial += 1
				for attrName in atomAttrs:
					try:
						...
						continue
		globalHelixOffset += localHelixMax
		globalStrandOffset += localStrandMax
		for b in m.bonds:
			a1, a2 = b.atoms
	...
	for opbg in PseudoBondMgr.mgr().pseudoBondGroups:
		if opbg in associatedModels:
			...
			pbg = combined.metalComplexGroup(issueHint=False)
