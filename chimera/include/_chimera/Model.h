...
# ifndef WrapPy
	...
	static Reason	SILHOUETTE_CHANGED;
...
# endif
	...
	void		addAssociatedModel(Model *model, bool propagateAttrs=true);
...
Model::setId(int i)
{
	if (id_ == i)
		return;
	if (id_ != OpenModels::Default && i != OpenModels::Default)
		throw std::logic_error("Model has already been assigned an id");
	id_ = i;
...
Model::setSubid(int i)
{
	if (subid_ == i)
		return;
	if (subid_ != OpenModels::Default && i != OpenModels::Default)
		throw std::logic_error("Model has already been assigned a subid");
	subid_ = i;
