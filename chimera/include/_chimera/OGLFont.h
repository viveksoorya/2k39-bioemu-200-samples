...
# ifndef WrapPy
	void		draw(const char *s, size_t len = 0) const;
...
# endif
	...
	void		draw(const std::string &str, size_t len = 0) const;
	float		width(const std::string &str, size_t len = 0) const;
	std::pair<float, float>
			height(const std::string &str, size_t len = 0) const;
	bool		valid() const;
...
inline void
OGLFont::draw(const std::string &str, size_t len) const
{
