...
public:
	...
	void		globalXform(const Xform &xf, bool allModels = false);
#ifndef WrapPy
	...
	static Reason	SOME_XFORMS_CHANGE;
	static Reason	XFORM_CHANGE;
	static Reason	XFORM_TRANSLATION;
...
private:
	...
	bool		computeBounds(Spheres *bspheres, BBox *box,
				      bool includeUndisplayedModels = false) const;
	void		notifyTranslation(const Xform &xf) const;
...
public:
	...
	static const int Default = INT_MIN;
	...
	size_t		count(bool inclEmpty = true, bool hidden = false, bool all = false) const;
	...
	enum CofrMethod	{ Fixed, CenterOfModels, Independent, CenterOfView,
	                  FrontCenter };
	Point		computeCofr(CofrMethod m) const;
...
private:
	...
	bool		xformChanged, someXformsChanged;
