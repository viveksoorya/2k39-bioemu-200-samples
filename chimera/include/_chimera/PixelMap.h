...
public:
	...
	PixelMap(Symbol name, int format, int type, int length);
	virtual ~PixelMap();
	// ATTRIBUTE: type
	int	type() const;
	// ATTRIBUTE: format
...
private:
	typedef std::map<PixelMap*, Symbol> BackMap;
	static NameMap	all;
	static BackMap	back;
# endif
private:
