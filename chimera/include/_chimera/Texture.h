...
public:
	...
	Texture(Symbol name, int format, int type, int width,
						int height = 1, int depth = 1);
	Texture(Symbol name, PixelMap* colormap, int type, int width,
						int height = 1, int depth = 1);
	virtual ~Texture();
...
# endif
	...
	void		filters(/*OUT*/ int *min, /*OUT*/ int *mag);
	void		setFilters(int min, int mag);
	void		sizes(/*OUT*/ int* w, /*OUT*/ int* h, /*OUT*/ int* d,
					/*OUT*/ int* nc, /*OUT*/ int* nb);
	Array<double, 4>	rgba(int index) const;
# ifndef WrapPy
	void		setRgba(int index, Array<double, 4> value);
# endif
	...
	void		setBaseMaterial(/*NULL_OK*/ Material *mat);
...
# endif
	...
	typedef std::map<Symbol, Texture*> NameMap;
	typedef std::pair<NameMap::const_iterator, NameMap::const_iterator> NameMapRange;
	static Texture* lookup(Symbol name) throw ();
...
private:
	typedef std::map<Texture* , Symbol> BackMap;
	static NameMap	all;
	static BackMap	back;
# endif
private:
	void		commonInit(Symbol name);
...
inline void
Texture::filters(int *min, int *mag)
{
	if (min)
		*min = minFilter;
	if (mag)
