...
//
// Define all inline functions after classes are all declared.
// Putting inline functions in each class header file causes incomplete
// type problems due to circular header file references.
//
namespace molecule {
...
inline void
Atom::setIdatmType(Symbol idatmType) {
...
Atom::setComputedIdatmType(const char *ct) {
	...
	for (std::vector<Atom *>::iterator li = locs.begin(); li != locs.end();
	++li)
		(*li)->computedIdatmType_ = Symbol(ct);
}
inline bool
...
Atom::shown() const
{
	return (display_ || hide_) && Molecule_->display();
}
inline const Color *
Atom::shownColor() const
{
	// The color of the current atom depiction
	if (hide_) {
		if (Residue_->hasRibbon()) {
			RibbonResidueClass *rrc = Residue_->ribbonResidueClass();
			const Atom *a = this;
			if (a->element() == Element::H)
				a = a->bondedToMainchain(rrc);
			if (a && rrc->hasPosition(a->name())) {
				const Color *color = Residue_->ribbonColor();
				if (color)
					return color;
				return Molecule_->color();
			}
		}
		// hidden but not a ribbon
	}
	if (color_)
		return color_;
	return Molecule_->color();
...
inline float
Atom::minimumLabelRadius() const
{
	return minimumLabelRadius_;
}
inline const Color *
...
inline Molecule::ResidueLabelPos
Molecule::residueLabelPos() const
{
	return residueLabelPos_;
}
inline const Color *
...
inline Molecule::RT
Molecule::ribbonType() const
{
	return ribbonType_;
}
inline void
Molecule::setRibbonType(Molecule::RT rt)
{
	if (ribbonType_ == rt)
		return;
	ribbonType_ = rt;
	invalidateRibbonData();
	setMajorChange();
	trackReason(RIBBON_TYPE_CHANGED);
}
inline double
Molecule::ribbonStiffness() const
{
	return ribbonStiffness_;
}
inline void
Molecule::setRibbonStiffness(double param)
{
	if (ribbonStiffness_ == param)
		return;
	ribbonStiffness_ = param;
	setMajorChange();
	trackReason(RIBBON_TYPE_CHANGED);
}
inline int
Molecule::ribbonSmoothing() const
{
	return ribbonSmoothing_;
}
inline void
Molecule::setRibbonSmoothing(int mask)
{
	if (ribbonSmoothing_ == mask)
		return;
	ribbonSmoothing_ = mask;
	invalidateRibbonData();
	setMajorChange();
	trackReason(RIBBON_TYPE_CHANGED);
}
...
inline void
PDBio::setBondLengthTolerance(Real t)
{
	tolerance = t;
}
inline bool
...
inline Symbol PseudoBond::category(void) const {
...
inline Symbol PseudoBondGroup:: category() const {
	return _category;
}
inline bool
...
PseudoBondMgr::pseudoBondGroups() const
{
	...
	for (std::map<Symbol, PseudoBondGroup *>::const_iterator i = PseudoBondGroups_.begin(); i != PseudoBondGroups_.end(); ++i)
		result.push_back(i->second);
	return result;
}
...
struct sort_coordIndex {
	bool operator()(const Atom* a1, const Atom* a2) { return a1->coordIndex() < a2->coordIndex(); }
};
...
Residue::atoms() const
{
	...
	for (std::multimap<Symbol, Atom *>::const_iterator i = Atoms_.begin(); i != Atoms_.end(); ++i)
		result.push_back(i->second);
	std::sort(result.begin(), result.end(), sort_coordIndex());
	return result;
}
...
inline void
Residue::setId(MolResId &mid)
{
	rid = mid;
	trackReason(MOLRESID_CHANGED);
}
inline bool
...
inline void
RibbonData::setCenter(const Point &c)
{
	center_ = c;
}
inline const Vector &
...
inline void
RibbonData::setNormal(const Vector &n)
{
	normal_ = n;
}
inline const Vector &
...
inline void
RibbonData::setBinormal(const Vector &bn)
{
	binormal_ = bn;
}
inline void
