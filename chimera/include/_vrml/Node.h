...
// --- UCSF Chimera Copyright ---
//
// $Id: Node.h 37548 2012-10-10 16:56:26Z goddard $
...
public:
	...
	const char *	typeName() const { return type_ ? type_->getName() : "UNKNOWN"; }
...
#ifdef OPENGL
public:
	virtual bool	render(const RenderOptions &/*ro*/)	{ return true; }
...
#ifdef OPENGL
	virtual bool	render(const RenderOptions &ro);
#endif
};
...
#ifdef OPENGL
class VRML_IMEX NodeDisplayList {
public:
		NodeDisplayList();
	virtual	~NodeDisplayList();
	bool	render(const RenderOptions &ro);
	void	newList(const RenderOptions &ro);
	void	endList(const RenderOptions &ro);
	bool	clear();
private:
	GLuint		listId;
};
