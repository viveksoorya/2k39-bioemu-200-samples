...
class SURFMODEL_IMEX SurfacePiece : public chimera::Selectable
{
 public:
  ...
  // geometry() returns vertices transformed by placement matrix, but
  // setGeometry() takes untransformed vertices.  Error prone.  TODO: Change this.
  ...
  int vertexCount() const;		// ATTRIBUTE: vertexCount

  // normals() are transformed by placement matrix, but
  // setNormal() takes untransformed vertices. Error prone. TODO: Change this.
  PyObject *normals() const;	// N by 3 array of vertex normal vectors
  void setNormals(PyObject *normals);

  PyObject *placement() const;	// 3x4 rotation and translation, None ok
  void setPlacement(PyObject *p);
  PyObject *unplacedVerticesAndNormals() const; // ATTRIBUTE: unplacedVerticesAndNormals
 ...
 private:
  ...
  static chimera::NotifierReason PLACEMENT_CHANGED;
  virtual void trackReason(const chimera::NotifierReason &reason) const;
