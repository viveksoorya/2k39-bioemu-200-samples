    ...
    def __getitem__(self, tag):
        try:
            ...
            if isinstance(td, list):
                # Create a list of tag values for repeated copies of the same tag
                dlist = []
                for type, data in td:
                    size, handler = self.load_dispatch[type]
                    dlist.append(handler(self, data))
                self.tags[tag] = dlist
            else:
                type, data = td
                size, handler = self.load_dispatch[type]
                self.tags[tag] = handler(self, data)
    ...
    def load(self, fp):
        ...
        for i in range(i16(fp.read(2))):
            ...
            if tag in self.tagdata:
                # Another copy of a tag that was already found.
                # Keep a list of tag values.
                td = self.tagdata[tag]
                if isinstance(td, list):
                    td.append(typ, data)
                else:
                    self.tagdata[tag] = [td, (typ, data)]
            else:
