...
def get_ccd(residue):
	...
	if f is not None:
		return f
	if residue in _negativeCache:
		return None
	try:
		data = _getWebService(residue)
		if not data:
			raise KeyError("no CCD")
	except:
		_negativeCache.add(residue)
		return None
...
def _getWebService(residue):
	url = "https://files.wwpdb.org/pub/pdb/refdata/chem_comp/" + residue[-1] + '/' + residue + '/' + residue + ".cif"
	print "Fetching %s template from" % residue, url
	import urllib2
