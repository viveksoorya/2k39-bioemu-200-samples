...
class CIFStructureBuilder(StructureBuilder.StructureBuilder):
    ...
    def read_start(self, filobj):
        ...
        self.cif = None
        for cif in cif_file.data_blocks:
            table = self._atom_site_table(cif)
            if table is None:
                continue
    ...
    def get_cell_parameters(self):
        ...
        try:
            ...
            alpha_degrees = 90
            warning("get_cell_parameters: alpha missing, using 90 degrees")
        try:
            beta_degrees = CIF.makeNumber(self.cif.tags["cell_angle_beta"])
        except ValueError:
            beta_degrees = 90
            warning("get_cell_parameters: beta missing, using 90 degrees")
        try:
            gamma_degrees = CIF.makeNumber(self.cif.tags["cell_angle_gamma"])
        except ValueError:
            gamma_degrees = 90
            warning("get_cell_parameters: gamma missing, using 90 degrees")
        z = self.cif.tags.get("cell_formula_units_z", None)
    ...
    def _atom_site_table(self, cif):
        """Return atom_site table (one with atom_site_label column)"""
        for t in cif.tables:
    ...
    def read_atoms(self):
        ...
        """
        table = self._atom_site_table(self.cif)
        if table is None:
