...
def _reporterror(func, args):
    ...
    if isinstance(exc_type, (type, types.ClassType)):
	# Handle python 1.5 class exceptions.
	exc_type = exc_type.__name__
    msg = unicode(exc_type) + ' Exception in Tk callback\n'
	...
	m = None
    try:
	sl = inspect.getsourcelines(func)[1]
    except (IOError, TypeError):
	sl = None
    msg = msg + '  Module: %s (line: %s)\n' % (repr(m), sl)
    msg = msg + '  Args: %s\n' % unicode(args)
	...
	msg = msg + '    %s\n' % tr[3]
    def convert(value):
	    # in function, so UnicodeDecodeError doesn't leak to sys.exc_info
	    try:
		return unicode(value)
	    except UnicodeDecodeError:
		return str(value).decode('utf-8', 'replace')
    msg = msg + u'%s: %s\n' % (exc_type, convert(exc_value))
