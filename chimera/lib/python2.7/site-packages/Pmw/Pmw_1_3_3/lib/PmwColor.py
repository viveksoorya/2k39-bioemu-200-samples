...
def bordercolors(root, colorName):
    ...
    r,g,b = name2rgb(root, colorName, 1)

    if r*0.5*r + g*1.0*g + b*0.28*b < _MAX_RGB*0.05*_MAX_RGB:
	darkRGB = [(_MAX_RGB + 3*c)/4 for c in (r,g,b)]
    else:
	darkRGB = [(60 * c)/100 for c in (r,g,b)]

    if g > _MAX_RGB*0.95:
	lightRGB = [(90 * c)/100 for c in (r,g,b)]
    else:
    	lightRGB = [max((_MAX_RGB + c)/2, min(_MAX_RGB, (14 * c)/10))
		    for c in (r,g,b)]
