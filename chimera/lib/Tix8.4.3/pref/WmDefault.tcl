...
#
#
#	$Id: WmDefault.tcl 38897 2013-06-27 05:01:27Z gregc $
#
...
#	known, one of: windows aqua gnome gnome2 kde1 kde2 cde kde
...
proc tixDetermineWM {} {
    # Returns one of cde kde1 kde2 gnome gnome2 windows aqua or ""
	...
	if {![catch {exec xlsatoms} xatoms]} {
		...
		set type gnome
	    } elseif {[string match *GNOME_PANEL_ACTION* $xatoms]} {
		set type gnome2
...
namespace eval ::wm_default {
   ...
   variable _nonzero_borderwidth_widgets {*Menubutton}

   # Widgets that are given a borderwidth of 0
   # must not be  *Entry
   variable _null_borderwidth_widgets {*Label}
   ...
   proc debug {str} {
    ...
    set _point [font actual TkDefaultFont -size]
	...
	variable \
		background		$tk::Palette(background) \
		foreground		$tk::Palette(foreground) \
		activebackground	$tk::Palette(activeBackground) \
		activeforeground	$tk::Palette(activeForeground) \
		disabledforeground	$tk::Palette(disabledForeground) \
		...
		disabledtextbackground	$tk::Palette(background) \
		selectbackground	$tk::Palette(selectBackground) \
		selectforeground	$tk::Palette(selectForeground) \
		selectcolor		$tk::Palette(selectColor) \
		highlightcolor		$tk::Palette(highlightColor) \
		highlightbackground	$tk::Palette(highlightBackground) \
		scrollbars		$tk::Palette(troughColor) \
		borderwidth		$_bd \
	...
	if {[tk windowingsystem] eq "aqua"} {
		...
		# On OS X 10.4, Courier is not a fixed width font!
		if {! [font metrics $_fixed_default -fixed]} {
		    set _fixed_default {Andale Mono}
		}
	    }
	}
	...
	variable \
		...
		activeforeground	$tk::Palette(activeForeground) \
		disabledforeground	$tk::Palette(disabledForeground) \
		disabledbackground	$tk::Palette(background) \
		textfamily		$_prop_default \
		systemfamily		$_prop_default \
		menufamily		$_prop_default \
		fixedfamily		$_fixed_default \
		...
		scrollbars		$tk::Palette(troughColor) \
		borderwidth		$_bd \
		menubackground		$tk::Palette(background) \
		menuforeground		$tk::Palette(foreground)
   ...
   proc default {args} {
    ...
    proc setup-aqua {args} {
	variable \
		menubackground		"SystemMenuBackground" \
		menuforeground		"SystemMenuText"
	set wm_default::textfamily "application"
	set wm_default::systemfamily "system"
	set wm_default::menufamily "system"
	set wm_default::fontsize [font actual system -size]
    }
    ...
    proc setup-gnome2 {args} {
    	# TODO: read ~/.gconf/desktop/gnome/interface/%gconf.xml
	# for font_name == application font

	set bg [option get . background Background]
	set fg [option get . foreground Foreground]
	if {![catch {exec gconftool-2 --get /desktop/gnome/interface/font_name} font]} {
	    set family [string trimright [lrange $font 0 end-1] ,]
	    set size [lindex $font end]
	    set wm_default::textfamily $family
	    set wm_default::systemfamily $family
	    set wm_default::menufamily $family
	    # Tk can only handle integer font sizes
	    set wm_default::fontsize [expr int($size)]
	}
	if {![catch {exec gconftool-2 --get /desktop/gnome/interface/monospace_font_name} fixed]} {
	    set family [lrange $fixed 0 end-1]
	    set wm_default::fixedfamily $family
	}
    ...
    proc pref-kde2 {file group key default} {
	exec kreadconfig --file $file --group $group --key $key --default $default
    }

    proc set-kde2-color {str var} {
	if {![catch {pref-kde2 kdeglobals General $str missing} color] && $color ne "missing"} {
    ...
    proc set-kde2-font {str var} {
	if {![catch {pref-kde2 kdeglobals General $str missing} font] && $font ne "missing"} {
	    set font [split $font ","]
	    set ::wm_default::$var [lindex $font 0]
	    debug "setting $var to $font"
	    set size [lindex $font 1]
	    if {$str eq "font" && [string is double -strict $size]} {
		# Tk can only handle integer font sizes
		set wm_default::fontsize [expr int($size)]
	    }
	}
	...
	set-kde2-color background background
	set-kde2-color foreground foreground
	set-kde2-color selectBackground selectbackground
	set-kde2-color selectForeground selectforeground
	set-kde2-color windowBackground textbackground
	set-kde2-color visitedLinkColor vlinkcolor
	set-kde2-color linkColor linkcolor
	set-kde2-font font textfamily
	set-kde2-font menuFont menufamily
	set wm_default::systemfamily $wm_default::textfamily
	set-kde2-font fixed fixedfamily

	if {![catch {pref-kde2 kwinrc Windows FocusPolicy ClickToFocus} policy] && $policy ne "ClickToFocus"} {
	    set wm_default::focusmodel focusfollowsmouse
	    debug "Setting focusmodel to $wm_default::focusmodel"
	}
    }
	...
	# until Tk 8.5 improves getting default fonts
	set smallsize [expr int($::wm_default::fontsize * .85 + .5)]
	font configure TkDefaultFont -family $::wm_default::systemfamily -size $::wm_default::fontsize
	font configure TkMenuFont -family $::wm_default::menufamily -size $::wm_default::fontsize
	font configure TkTextFont -family $::wm_default::textfamily -size $::wm_default::fontsize
	font configure TkFixedFont -family $::wm_default::fixedfamily -size $::wm_default::fontsize
	font configure TkHeadingFont -family $::wm_default::systemfamily -size $smallsize
	font configure TkCaptionFont -family $::wm_default::systemfamily -size $::wm_default::fontsize -weight bold
	font configure TkSmallCaptionFont -family $::wm_default::systemfamily -size $::wm_default::fontsize
	font configure TkIconFont -family $::wm_default::systemfamily -size $::wm_default::fontsize
	font configure TkTooltipFont -family $::wm_default::systemfamily -size $smallsize
    }
	...
	if {$::wm_default::focusmodel ne "clicktofocus"} {
